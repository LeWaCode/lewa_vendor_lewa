 ...
 */
public class SpecialNumberPhotoLoader implements Callback {

    private static final String LOADER_THREAD_NAME = "SpecialNumberPhotoLoader";
    ...
    private final String[] COLUMNS = new String[] {"number", "logo"};
     ...
     */
    public SpecialNumberPhotoLoader(Context context, int defaultResourceId) {
     ...
     * from the database.
     */
    public void loadPhoto(ImageView view, String number) {
        ...
        if (number.length() <= 2) {
            ...
            boolean loaded = loadCachedPhoto(view, number);
            if (loaded) {
                mPendingRequests.remove(view);
            } else {
     ...
     */
    private boolean loadCachedPhoto(ImageView view, String number) {
        BitmapHolder holder = mBitmapCache.get(number);
        if (holder == null) {
            holder = new BitmapHolder();
            mBitmapCache.put(number, holder);
     ...
     */
    private void cacheBitmap(String number, byte[] bytes) {
        if (mPaused) {
            return;
        }
    ...
    private class LoaderThread extends HandlerThread implements Callback {
        ...
        private void loadPhotosFromDatabase() {
            ...
            for (int i = 0; i < count; i++) {
                cacheBitmap(mNumbers.get(i), null);
            }
        }
    }
}

...
public class ContactPhotoLoader implements Callback {
    ...
    private final String[] COLUMNS = new String[] { Photo._ID, Photo.DATA15}; //Photo.PHOTO };
    ...
    private final int mUnknowResourceId;
    ...
    private final ConcurrentHashMap<String, BitmapHolder> mSpecialBitmapCache =
    ...
    private final ConcurrentHashMap<ImageView, String> mSpecialPendingRequests =
    ...
    public ContactPhotoLoader(Context context, int defaultResourceId, int unknowResourceId) {
        mDefaultResourceId = defaultResourceId;
        mUnknowResourceId = unknowResourceId;
        mContext = context;
    }

    public void loadPhoto(ImageView view, long photoId, long contactId) {
        if ((contactId <= 0) && (0 == photoId)) {
            view.setScaleType(ImageView.ScaleType.FIT_XY);
            view.setImageResource((mUnknowResourceId > 0)? mUnknowResourceId : mDefaultResourceId);
            mPendingRequests.remove(view);
        }
        else {
            loadPhoto(view, photoId);
        }
    }
    ...
    public void loadPhoto(ImageView view, long photoId) {
        mSpecialPendingRequests.remove(view);
    ...
    public void loadSpecialPhoto(ImageView view, String number) {
        mPendingRequests.remove(view);
        ...
        if (number.length() <= 2) {
            ...
            boolean loaded = loadCachedSpecialPhoto(view, number);
            if (loaded) {
                mSpecialPendingRequests.remove(view);
            } else {
                mSpecialPendingRequests.put(view, number);
                if (!mPaused) {
                    // Send a request to start loading photos
                    requestLoading();
                }
    ...
    private boolean loadCachedSpecialPhoto(ImageView view, String number) {
        BitmapHolder holder = mSpecialBitmapCache.get(number);
        if (holder == null) {
            holder = new BitmapHolder();
            mSpecialBitmapCache.put(number, holder);
        } else if (holder.state == BitmapHolder.LOADED) {
            // Null bitmap reference means that database contains no bytes for the photo
            if (holder.bitmapRef == null) {
                view.setImageResource(((mUnknowResourceId > 0)? mUnknowResourceId : mDefaultResourceId));
    ...
    public void clear() {
        ...
        mSpecialPendingRequests.clear();
        mSpecialBitmapCache.clear();
    }
    ...
    public void resume() {
        mPaused = false;
        if (!mPendingRequests.isEmpty() || !mSpecialPendingRequests.isEmpty()) {
            requestLoading();
        }
    }
    ...
    private void processLoadedImages() {
        ...
        iterator = mSpecialPendingRequests.keySet().iterator();
        while (iterator.hasNext()) {
            ImageView view = iterator.next();
            String number = mSpecialPendingRequests.get(view);
            boolean loaded = loadCachedSpecialPhoto(view, number);
            if (loaded) {
                iterator.remove();
            }
    ...
    private void cacheBitmap(long id, byte[] bytes) {
        ...
        if (bytes != null) {
            try {
                ...
                bitmap = ImageUtil.toRoundCorner(bitmap, 30);
    ...
    private void cacheSpecialBitmap(String number, byte[] bytes) {
        if (mPaused) {
            return;
        }
    ...
    private void obtainNumbersToLoad(ArrayList<String> numbers) {
         ...
         * is complete.
         */
        Iterator<String> iterator = mSpecialPendingRequests.values().iterator();
        while (iterator.hasNext()) {
    ...
    private class LoaderThread extends HandlerThread implements Callback {
        ...
        private final ArrayList<Long> mPhotoIds = new ArrayList<Long>(); //Lists.newArrayList();
        private final ArrayList<String> mPhotoIdsAsStrings = new ArrayList<String>(); //Lists.newArrayList();
        ...
        private void loadSpecialPhotosFromDatabase() {
            ...
            Cursor cursor = null;
            try {
                cursor = mResolver.query(Uri.parse("content://com.lewa.providers.location/special_phone"),
                        new String[] {"number", "logo"},
                        "number IN(" + Util.numberArrayToInClauseString(mNumbers, "+86", null) + ")",
                        null,
                        null);
                ...
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        ...
                        byte[] bytes = cursor.getBlob(1);
                        cacheSpecialBitmap(number, bytes);
            ...
            count = mNumbers.size();
            for (int i = 0; i < count; i++) {
                cacheSpecialBitmap(mNumbers.get(i), null);
            }
        }
    }
}

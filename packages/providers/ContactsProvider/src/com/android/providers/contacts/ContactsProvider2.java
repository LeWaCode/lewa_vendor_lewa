...
public class ContactsProvider2 extends SQLiteContentProvider implements OnAccountsUpdateListener {
    ...
    //for yiliao, add by jxli
    private static final int USER = 17001;
    private static final int ROSTER = 17002;
    private static final int ROSTER_DATA = 17003;
    private static final int ROSTER_ID = 17004;
    private static final int ROSTER_DATA_ID = 17005;
    private static final int USER_ID = 17006;
    private static final int ROSTER_FILTER = 17007;
    ...
    //** Contains Roster columns */
    private static final HashMap<String, String> sRosterProjectionMap;
    //** Contains Rosterdata columns */
    private static final HashMap<String, String> sRosterdataProjectionMap;
    //end
    ...
    static {
	 ...
	 matcher.addURI(ContactsContract.AUTHORITY, "raw_contacts/#/display_photo",
        ...
        //for yiliao, add by jxli
        matcher.addURI(ContactsContract.AUTHORITY, "user", USER);
        matcher.addURI(ContactsContract.AUTHORITY, "user/#", USER_ID);
        matcher.addURI(ContactsContract.AUTHORITY, "roster", ROSTER);
        matcher.addURI(ContactsContract.AUTHORITY, "roster/#", ROSTER_ID);
        matcher.addURI(ContactsContract.AUTHORITY, "rosterdata", ROSTER_DATA);
        matcher.addURI(ContactsContract.AUTHORITY, "rosterdata/#", ROSTER_DATA_ID);
        matcher.addURI(ContactsContract.AUTHORITY, "roster/filter/*", ROSTER_FILTER);
        //end
    }
    ...
    static {
        ...
        sContactsProjectionMap.put(Contacts.CONTACT_TYPE, Contacts.CONTACT_TYPE); //for yiliao, add by jxli
        sContactsProjectionMap.put(Contacts.YL_REMARK, Contacts.YL_REMARK); //for yiliao, add by jxli
        sContactsProjectionMap.put(Contacts.PHOTO_FILE_ID, Contacts.PHOTO_FILE_ID);//for sticker photo,add by hyzeng
        ...
        sRawContactsProjectionMap.put(RawContacts.CONTACT_TYPE, RawContacts.CONTACT_TYPE);  //for yiliao, add by jxli
        sRawContactsProjectionMap.put(RawContacts.YL_REMARK, RawContacts.YL_REMARK); //for yiliao, add by jxli
        ...
        sUserProjectionMap.put(User.USER_ID, User.USER_ID);
        sUserProjectionMap.put(User.NICK_NAME, User.NICK_NAME);
        sUserProjectionMap.put(User.PASSWD, User.PASSWD);
        sUserProjectionMap.put(User.PHOTO, User.PHOTO);
        sUserProjectionMap.put(User.REMARK, User.REMARK);
        sUserProjectionMap.put(User.EMAIL, User.EMAIL);
        sUserProjectionMap.put(User.STATUS, User.STATUS);
        sUserProjectionMap.put(User.ACCOUNT_TYPE, User.ACCOUNT_TYPE);
        //add by zenghuaying for weibo
        sUserProjectionMap.put(User.WEIBO_NAME, User.WEIBO_NAME);
        sUserProjectionMap.put(User.WEIBO_STATUS, User.WEIBO_STATUS);
        //add end
        
        sRosterProjectionMap = new HashMap<String, String>();
        sRosterProjectionMap.put(Roster._ID, Roster._ID);
        sRosterProjectionMap.put(Roster.CONTACT_ID, Roster.CONTACT_ID);
        sRosterProjectionMap.put(Roster.NAME_USER_ID, Roster.NAME_USER_ID);
        sRosterProjectionMap.put(Roster.DISPLAY_NAME, Roster.DISPLAY_NAME);
        sRosterProjectionMap.put(Roster.SORT_KEY, Roster.SORT_KEY);
        sRosterProjectionMap.put(Roster.DURATION, Roster.DURATION);
        sRosterProjectionMap.put(Roster.REMARK, Roster.REMARK);
        sRosterProjectionMap.put(Roster.PHOTO, Roster.PHOTO);
        sRosterProjectionMap.put(Roster.STATUS, Roster.STATUS);
        sRosterProjectionMap.put(Roster.IS_DEFAULT, Roster.IS_DEFAULT);
                
        sRosterdataProjectionMap = new HashMap<String, String>();
        sRosterdataProjectionMap.put(RosterData._ID, RosterData._ID);
        sRosterdataProjectionMap.put(RosterData.ROSTER_ID, RosterData.ROSTER_ID);
        sRosterdataProjectionMap.put(RosterData.ROSTER_USER_ID, RosterData.ROSTER_USER_ID);
        sRosterdataProjectionMap.put(RosterData.NAME_USER_ID, RosterData.NAME_USER_ID);
        sRosterdataProjectionMap.put(RosterData.NICK_NAME, RosterData.NICK_NAME);
        sRosterdataProjectionMap.put(RosterData.DISPLAY_NAME, RosterData.DISPLAY_NAME);
        sRosterdataProjectionMap.put(RosterData.EMAIL, RosterData.EMAIL);
        sRosterdataProjectionMap.put(RosterData.PHOTO, RosterData.PHOTO);
        sRosterdataProjectionMap.put(RosterData.SORT_KEY, RosterData.SORT_KEY);
        sRosterdataProjectionMap.put(RosterData.DURATION, RosterData.DURATION);
        sRosterdataProjectionMap.put(RosterData.REMARK, RosterData.REMARK);
        sRosterdataProjectionMap.put(RosterData.STATUS, RosterData.STATUS);
        sRosterdataProjectionMap.put(RosterData.IS_PRIMARY, RosterData.IS_PRIMARY);
        //end
    }
    ...
    private abstract class DataRowHandler {
        ...
        public long insert(SQLiteDatabase db, long rawContactId, ContentValues values) {
            ...
            Log.e(TAG, "insert: " + rawContactId);
            Integer primary = values.getAsInteger(Data.IS_PRIMARY);
            if (primary != null && primary != 0) {
                setIsPrimary(rawContactId, dataId, getMimeTypeId());
            }
            Log.e(TAG, "insert: contactId = " + mDbHelper.getContactId(rawContactId));
            //update Contact_type for yiliao, add by jxli
            if (values.size() > 0 && mMimetype.equals(Phone.CONTENT_ITEM_TYPE) == true) {
                String phone = values.getAsString(Data.DATA1);
                Log.e(TAG, "insert: phone = " + phone);
                updateContactType(null, phone, rawContactId);
            } else if (values.size() > 0 && mMimetype.equals(Photo.CONTENT_ITEM_TYPE) == true) {
                updateRosterPhoto(rawContactId, values);
            }
            //end
            return dataId;
        }
        ...
        public boolean update(SQLiteDatabase db, ContentValues values, Cursor c,
            ...
            if (values.containsKey(Data.IS_SUPER_PRIMARY)) {
                ...
                //Deleted by GanFeng 20120207, fix bug1785
            ...
            if (values.size() > 0) {
                ...
                //get old number for yiliao, add by jxli
                if (mMimetype.equals(Phone.CONTENT_ITEM_TYPE) == true) {
                    Cursor cursor = mDb.query(Tables.DATA, new String[]{Data.DATA1}, Data._ID + " =?", mSelectionArgs1, null, null, null, null);
                    try {
                        if (cursor.moveToFirst()) {
                            oldNumber = cursor.getString(0);
                        }
                    } finally {
                        cursor.close();
                    }
                ...
                if (mMimetype.equals(Phone.CONTENT_ITEM_TYPE) == true) {
                    String phone = values.getAsString(Data.DATA1);
                    Log.e(TAG, "update: phone = " + phone + "; rawContactId = " + rawContactId);
                    updateContactType(oldNumber, phone, rawContactId);
                } else if (mMimetype.equals(Photo.CONTENT_ITEM_TYPE) == true) {
                    updateRosterPhoto(rawContactId, values);
                }
                //end
            }
        ...
        public int delete(SQLiteDatabase db, Cursor c) {
            ...
            mSelectionArgs1[0] = String.valueOf(dataId);
            //for yiliao, add by jxli
            Cursor cursor = mDb.query(Tables.DATA, new String[]{Data.DATA1}, Data._ID + " =?" + " AND "
                            + DataColumns.MIMETYPE_ID + "='" + mDbHelper.getMimeTypeId(Phone.CONTENT_ITEM_TYPE) + "'", mSelectionArgs1, null, null, null, null);
            try {
                if (cursor.moveToFirst()) {
                    oldNumber = cursor.getString(0);
                }
            ...
            //for yiliao, add by jxli
            updateContactType(oldNumber, null, rawContactId);
            //end
            return count;
        }
        ...
        //for yiliao, add by jxli
        private void updateContactType(String oldNum, String phoneNum, long rawContactId) { 
            Cursor cursor = null;
            ContentValues cv = new ContentValues();
            String strOldNum = oldNum;
            String strPhoneNum = phoneNum;
            long rosterId = 0;
            
            if (oldNum != null) {
                strOldNum = strOldNum.replace("+86", "");
                if (null != strOldNum && strOldNum.length() > 0)
                    strOldNum = strOldNum.replace("-", "");
                cursor = query(RosterData.CONTENT_URI, new String[]{RosterData.ROSTER_ID}, RosterData.ROSTER_USER_ID + "=\'" + strOldNum + "\'", null, null);
                try {
                    if (cursor.moveToFirst()) {
                        cv.put(Roster.CONTACT_ID, 0);
                        ContactsProvider2.this.update(Roster.CONTENT_URI, cv, Roster._ID + "=" + cursor.getLong(0), null);
                        updateRosterContactId(strOldNum, cursor.getLong(0));
                    }
                } finally {
                    cursor.close();
                    cursor = null;
            ...
            if (null != strPhoneNum && strPhoneNum.length() > 0)
                strPhoneNum = strPhoneNum.replace("+86", "");
            if (null != strPhoneNum && strPhoneNum.length() > 0)
                strPhoneNum = strPhoneNum.replace("-", "");
            if (null != strPhoneNum && strPhoneNum.length() > 0)
            {
                cursor = query(RosterData.CONTENT_URI, new String[]{RosterData.ROSTER_ID, RosterData.REMARK}, RosterData.ROSTER_USER_ID + "=\'" + strPhoneNum + "\'", null, null);
                Log.e(TAG, "updateContactType(), phoneNum = " + strPhoneNum + ", rawContactId = " + rawContactId);
                try {
                    if (cursor.moveToFirst()) {
                        Log.e(TAG, "updateContactType(), contactType = 1");
                        contactType = 1;
                        remark = cursor.getString(1);
                        rosterId = cursor.getLong(0);
                        cv.clear();
                       // if (oldNum == null || oldNum.length() > 0) {
                            Cursor rosterCursor = query(Roster.CONTENT_URI, new String[]{Roster.CONTACT_ID}, Roster._ID + "=" + rosterId, null, null);
                            try {
                                if (rosterCursor.moveToFirst() && 0 == rosterCursor.getLong(0)) {
                                    cv.put(Roster.CONTACT_ID, rawContactId);
                                    ContactsProvider2.this.update(Roster.CONTENT_URI, cv, Roster._ID + "=" + rosterId, null);
                                }
                            } finally {
                                rosterCursor.close();
                                rosterCursor = null;
                    ...
                    //update contactType for insert new phone
                    if(oldNum == null) {
	                    if (contactType == 1 ) {
	                        cv.clear();
	                        cv.put(RawContacts.CONTACT_TYPE, contactType);
	                        cv.put(RawContacts.YL_REMARK, remark);
	                        ContactsProvider2.this.update(RawContacts.CONTENT_URI, cv, RawContacts._ID + "=" + rawContactId, null);                 
	                    }
			     return;
	            	}
                } finally {
            ...
            //update contactType for update phone
            cursor = query(Data.CONTENT_URI, new String[]{Phone.NUMBER}, Data.RAW_CONTACT_ID + "=" + rawContactId + " AND "
                            + Data.MIMETYPE + "='" + Phone.CONTENT_ITEM_TYPE + "'", null, null);
            try {
                String strWhere = new String("");
                
                if (cursor.moveToFirst()) {
                    strWhere += "\'" + cursor.getString(0) + "\'"; 
                }
                ...
                if (strWhere.length() > 0) {
                    Cursor rosterCursor = query(RosterData.CONTENT_URI, new String[]{RosterData.ROSTER_ID, RosterData.REMARK}, RosterData.ROSTER_USER_ID + " in (" + strWhere + ")", null, null);
                    try {
                        if (rosterCursor.moveToFirst()) {
                            contactType = 1;
                            remark = rosterCursor.getString(1);
                            cv.clear();
                            cv.put(Roster.CONTACT_ID, rawContactId);
                            ContactsProvider2.this.update(Roster.CONTENT_URI, cv, Roster._ID + "=" + rosterCursor.getLong(0), null);
                        } else {
                            contactType = 0;
                            remark = "";
                            updateRosterContactId(strPhoneNum, rosterId);
                        }
                        cv.clear();
                        cv.put(RawContacts.CONTACT_TYPE, contactType);
                        cv.put(RawContacts.YL_REMARK, remark);
        ...
        private void updateRosterContactId(String phone, long rosterId) {
            long rawContactId = 0;
            String phoneNumber = phone;
            String strippedNumber = "REPLACE(TRIM(" + Phone.NUMBER + "), '-', '')";
            String strWhere = null;
            ContentValues cv = new ContentValues();
            
            if (phoneNumber != null && !phoneNumber.startsWith("+86")) {
                strWhere = strippedNumber + "='" + phoneNumber + "'"
                        + " OR "
                        + strippedNumber + "='" + "+86" + phoneNumber + "'";
            }
            //Uri uri = Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, Uri.encode(phoneNumber));
            Cursor cursor = query(Phone.CONTENT_URI, new String[]{Phone.CONTACT_ID}, strWhere, null, null);
            try {
                while (cursor.moveToNext()) {
                    long contactId = cursor.getLong(0);
                    Cursor rawcontactCursor = query(RawContacts.CONTENT_URI, new String[]{RawContacts._ID}, RawContacts.CONTACT_ID + "=" + contactId, null, null);
                    if (rawcontactCursor != null && rawcontactCursor.moveToFirst()) {
                        rawContactId = rawcontactCursor.getLong(0);
                        cv.put(Roster.CONTACT_ID, rawContactId);
                    }
                    rawcontactCursor.close();
                    rawcontactCursor = null;
                    ContactsProvider2.this.update(Roster.CONTENT_URI, cv, Roster._ID + "=" + rosterId, null);
                }
            } finally {
                cursor.close();
        ...
        private void updateRosterPhoto(long rawContactId, ContentValues values) {
            ArrayList<Long> arrRosterIds = new ArrayList<Long>(); 
            Cursor cursor = query(Roster.CONTENT_URI, new String[]{Roster._ID}, Roster.CONTACT_ID+"="+rawContactId, null, null);
            try {
                while (cursor.moveToNext()) {
                    arrRosterIds.add(cursor.getLong(0));
                }
            } finally {
                cursor.close();
                cursor = null;
            ...
            int size = arrRosterIds.size();
            if (arrRosterIds.size() > 0) {
                for (int i = 0; i < size; i++) {
                    boolean bHasPhoto = false;
                    cursor = query(RosterData.CONTENT_URI, new String[]{RosterData.PHOTO}, RosterData.ROSTER_ID+"="+arrRosterIds.get(i), null, null);
                    try {
                        while (cursor.moveToNext()) {
                    ...
                    if (!bHasPhoto) {
                        ContentValues cv = new ContentValues();
                        cv.put(Roster.PHOTO, values.getAsByteArray(Photo.PHOTO));
                        ContactsProvider2.this.update(Roster.CONTENT_URI, cv, Roster._ID + "=" + arrRosterIds.get(i), null);
                        cv = null;
                    }
                }
            }
    ...
    public DataRowHandlerForPhoto(
        ...
        super(DisplayPhoto.CONTENT_ITEM_TYPE);
    ...
    public long insert(SQLiteDatabase db,  long rawContactId, ContentValues values) {
        ...
        long dataId = super.insert(db,  rawContactId, values);
        if (!isNewRawContact(rawContactId)) {
            mContactAggregator.updatePhotoFileId(db, rawContactId);
        }
        return dataId;
    }
    ...
    private boolean preProcessPhoto(ContentValues values) {
        if (values.containsKey(Photo.PHOTO)) {
            ...
            if (photoExists) {
                ...
                values.putNull(DisplayPhoto.PHOTO);
                values.putNull(DisplayPhoto.PHOTO_FILE_ID);
            }
        }
        return true;
    }
    ...
    private boolean processPhoto(ContentValues values) {
        ...
        if (originalPhoto != null) {
            int maxDisplayPhotoDim = getContext().getResources().getInteger(
                    R.integer.config_max_display_photo_dim);
            int maxThumbnailPhotoDim = getContext().getResources().getInteger(
            ...
            try {
                ...
                if (photoFileId != 0) {
                    values.put(DisplayPhoto.PHOTO_FILE_ID, photoFileId);
                } else {
                    values.putNull(DisplayPhoto.PHOTO_FILE_ID);
                }
                values.put(DisplayPhoto.PHOTO, processor.getThumbnailPhotoBytes());
                return true;
    ...
    private void initDataRowHandlers() {
      ...
      mDataRowHandlers.put(DisplayPhoto.CONTENT_ITEM_TYPE, new DataRowHandlerForPhoto(getContext(), mDbHelper, mContactAggregator, mContactsPhotoStore));
    }
    /**
     * Visible for testing.
     */
    ...
    private void initForDefaultLocale() {
	 ...
	 mContactsPhotoStore = new PhotoStore(getContext().getFilesDir(), mDbHelper);
        initDataRowHandlers();
    }
   ...
   protected void cleanupPhotoStore(){
        ...
        long photoMimeTypeId = mDbHelper.getMimeTypeId(DisplayPhoto.CONTENT_ITEM_TYPE);
        Cursor c = db.query(Views.DATA_ALL, new String[]{Data._ID, DisplayPhoto.PHOTO_FILE_ID},
                DataColumns.MIMETYPE_ID + "=" + photoMimeTypeId + " AND "
                        + DisplayPhoto.PHOTO_FILE_ID + " IS NOT NULL", null, null, null, null);
        ...
        Set<Long> missingPhotoIds = mContactsPhotoStore.cleanup(usedPhotoFileIds);
    ...
    protected void notifyChange(boolean syncToNetwork) {
        //zhangwei -yl
        if (mMatch == ROSTER_DATA){
            getContext().getContentResolver().notifyChange(RosterData.CONTENT_URI, null,
                    syncToNetwork);   
            mMatch = 0;
        }else{
    ...
    protected Uri insertInTransaction(Uri uri, ContentValues values) {
        ...
        switch (match) {
            ...
            case USER: {
                id = insertUserData(values);
                break;
            }

            case ROSTER: {
                id = insertRoster(values);
                break;
            }

            case ROSTER_DATA: {
                id = insertRosterData(values);
                break;
            }
    ...
    private long insertUserData(ContentValues values) {
        long id = mDb.insert(Tables.USER, User.USER_ID, values);
        return id;
    }

    private long insertRoster(ContentValues values) {
        long id = mDb.insert(Tables.ROSTER, Roster.DISPLAY_NAME, values);
        return id;
    }

    private long insertRosterData(ContentValues values) {
        String sort_key = null;
        long rawContactId = 0;
        ContentValues cv = new ContentValues();
        Cursor cursor = null;
         
        if (values.containsKey(RosterData.DISPLAY_NAME)) {
            sort_key = ContactLocaleUtils.getIntance().getSortKey((String)values.get(RosterData.DISPLAY_NAME), FullNameStyle.CHINESE);
        } else if (values.containsKey(RosterData.NICK_NAME)) {
            String nick_name = (String)values.get(RosterData.NICK_NAME);
            sort_key = ContactLocaleUtils.getIntance().getSortKey(nick_name, FullNameStyle.CHINESE);
            values.put(RosterData.DISPLAY_NAME, nick_name);
        } else if (values.containsKey(RosterData.ROSTER_USER_ID)) {
/*            String display_name = (String)values.get(RosterData.ROSTER_USER_ID);
            Uri uri = Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, Uri.encode(display_name));
            cursor = query(uri, new String[]{PhoneLookup.DISPLAY_NAME}, null, null, null);
            try {
                if (cursor.moveToFirst()) {
                    display_name = cursor.getString(0);
                }
            } finally {
                cursor.close();
                cursor = null;
            }
            sort_key = ContactLocaleUtils.getIntance().getSortKey(display_name, FullNameStyle.CHINESE);*/
            //values.put(RosterData.DISPLAY_NAME, display_name);
            sort_key = values.getAsString(RosterData.ROSTER_USER_ID);
            values.put(RosterData.DISPLAY_NAME, (String)values.get(RosterData.ROSTER_USER_ID));
        }
        values.put(RosterData.SORT_KEY, sort_key);
        cv.put(Roster.SORT_KEY, sort_key);
        if (values.containsKey(RosterData.NAME_USER_ID)) cv.put(Roster.NAME_USER_ID, values.getAsInteger(RosterData.NAME_USER_ID));
        if (values.containsKey(RosterData.DISPLAY_NAME)) cv.put(Roster.DISPLAY_NAME, values.getAsString(RosterData.DISPLAY_NAME));    
        if (values.containsKey(RosterData.REMARK)) cv.put(Roster.REMARK, values.getAsString(RosterData.REMARK));
        if (values.containsKey(RosterData.ROSTER_USER_ID)) {
            String phoneNumber = (String)values.get(RosterData.ROSTER_USER_ID);
            String strippedNumber = "REPLACE(TRIM(" + Phone.NUMBER + "), '-', '')";
            String strWhere = null;
            ...
            Log.e(TAG, "strWhere = " + strWhere);
            //Uri uri = Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, Uri.encode(phoneNumber));
            cursor = query(Phone.CONTENT_URI, new String[]{Phone.CONTACT_ID}, strWhere, null, null);
            try {
                while (cursor.moveToNext()) {
                    ...
                    rawcontactCursor.close();
                    rawcontactCursor = null;
                    //update Contats.CONTACT_TYPE to yiliao
                    ContentValues contactvalues = new ContentValues();
                    contactvalues.put(Contacts.CONTACT_TYPE, 1);
                    updateContactOptions(contactId, contactvalues);
                }
            } finally {
                cursor.close();
                cursor = null;
        ...
        if (values.containsKey(RosterData.PHOTO)) {
            cv.put(Roster.PHOTO, values.getAsByte(RosterData.PHOTO));
        } else if (rawContactId != 0) {
            cursor = query(Data.CONTENT_URI, new String[]{Data.MIMETYPE, Photo.PHOTO, StructuredName.DISPLAY_NAME, Contacts.DISPLAY_NAME, Contacts.SORT_KEY_PRIMARY}, 
                Data.RAW_CONTACT_ID+"="+rawContactId + " AND ("
                  + Data.MIMETYPE + "='" + Photo.CONTENT_ITEM_TYPE + "' OR " + Data.MIMETYPE + "='" + StructuredName.CONTENT_ITEM_TYPE + "')", null, null);
            try {
                while (cursor.moveToNext()) {
                    if (cursor.getString(0).equals(Photo.CONTENT_ITEM_TYPE)) {
                        byte[] bytes = cursor.getBlob(1);
                        if (bytes != null && bytes.length > 0) {
                            values.put(RosterData.PHOTO, bytes);
                            cv.put(Roster.PHOTO, bytes);
                        }
                    } else if (cursor.getString(0).equals(StructuredName.CONTENT_ITEM_TYPE)) {
                        String name = cursor.getString(2);
                        if (name != null) {
                            cv.put(Roster.DISPLAY_NAME, cursor.getString(3));
                            cv.put(Roster.SORT_KEY, cursor.getString(4));
                        }
                    }
                }
            } finally {
...
//        if (values.containsKey(RosterData.IS_LOCAL)) cv.put(Roster.IS_LOCAL, values.get(RosterData.IS_LOCAL)); 
        long roster_id = insertRoster(cv);
        cv = null;    
        
        values.put(RosterData.ROSTER_ID, new Integer((int)roster_id));
//        values.remove(RosterData.IS_LOCAL);
        long id = mDb.insert(Tables.ROSTER_DATA, RosterData.NAME_USER_ID, values); 
    ...
    private long insertRawContact(Uri uri, ContentValues values) {
        ...
        long contactId = mDbHelper.getContactId(rawContactId);
        Log.e(TAG, "insertRawContact: rawContactId = " + rawContactId + ", contactId = " + contactId);
        return rawContactId;
    }
    ...
    private long insertData(ContentValues values, boolean callerIsSyncAdapter) {
        ...
        Log.e(TAG, "insertData: contactId = " + mDbHelper.getContactId(rawContactId));
        return id;
    }
    ...
    public void updateRawContactDisplayName(SQLiteDatabase db, long rawContactId) {
        ...
        //Modified by GanFeng 20120203, fix bug3095
        ...
        //} else {
            ...
            //Added by GanFeng 20120116, fix bug3095
            if ((FullNameStyle.WESTERN == displayNameStyle) && (null != displayNamePrimary)) {
                int nameLength = displayNamePrimary.length();
                for (int i = 0; i < nameLength; ++i) {
                    if (displayNamePrimary.charAt(i) > 128) { //consider as CHINESE
        ...
        //}
        //Ended modifying by GanFeng 20120203
    ...
    protected int deleteInTransaction(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case USER: {
//                mDb.delete(Tables.ROSTER_DATA, RosterData.NAME_USER_ID, selectionArgs);
//                mDb.delete(Tables.ROSTER, Roster.NAME_USER_ID, selectionArgs);
                return mDb.delete(Tables.USER, selection, selectionArgs);
                
            }

            case ROSTER: {                
                mDb.delete(Tables.ROSTER_DATA, RosterData.ROSTER_ID, selectionArgs);
                return mDb.delete(Tables.ROSTER, selection, selectionArgs);
            }
    ...
    private int deleteContact(long contactId) {
        ...
        try {
            while (c.moveToNext()) {
                ...
                ContentValues cv = new ContentValues();
                boolean bSet = false;
                Cursor rosterCursor = query(Roster.CONTENT_URI, new String[]{Roster._ID}, Roster.CONTACT_ID + "=" + rawContactId, null, null);
                try {
                    if (rosterCursor.moveToFirst())
                    {
                        Cursor cursor = query(RosterData.CONTENT_URI, new String[]{RosterData.ROSTER_USER_ID}, RosterData.ROSTER_ID + "=" + rosterCursor.getLong(0), null, null);
                        try {     
                            while (cursor.moveToNext()) {
                                ...
                                Log.e(TAG, "deleteContact(), 3 number = " + strWhere);
                                Cursor dataCursor = query(Data.CONTENT_URI, new String[]{Data.RAW_CONTACT_ID, Contacts.DISPLAY_NAME}, strWhere, null, null);
                                try {
                                    if (dataCursor.moveToFirst()) {
                                        cv.put(Roster.CONTACT_ID, dataCursor.getLong(0));
                                        update(Roster.CONTENT_URI, cv, Roster.CONTACT_ID + "=" + rawContactId, null);
                                        bSet = true;
                                        break;
                                    }
                                } finally {
    ...
    protected int updateInTransaction(Uri uri, ContentValues values, String selection,
        ...
        switch(match) {
            ...
            case USER: {
                count = mDb.update(Tables.USER, values, selection, selectionArgs);
                break;
            }

            case ROSTER: {
                count = updateRoster(uri, values, selection, selectionArgs);
                break;
            }

            case ROSTER_DATA: {
                mMatch = match;
                count = updateRosterData(uri, values, selection, selectionArgs);              
                break;
            }             
      ...
      private int updateRoster(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        Cursor cursor = null;
        if (values != null && values.containsKey(Roster.CONTACT_ID)) {
            long rawContactId = values.getAsLong(Roster.CONTACT_ID);
            cursor = query(Data.CONTENT_URI, new String[]{Data.DATA1, Contacts.DISPLAY_NAME, Contacts.SORT_KEY_PRIMARY},
                        Data.RAW_CONTACT_ID + "=" + rawContactId + " AND "
                        + Data.MIMETYPE + "='" + StructuredName.CONTENT_ITEM_TYPE + "'", null, null);
            try {
                if (cursor.moveToFirst()) {
                    String name = cursor.getString(0);
                    Log.e(TAG, "updateRoster() 2 name: " + name);
                    if (name != null) {
                        Log.e(TAG, "updateRoster() 1 rawContactId: " + rawContactId);
                        values.put(Roster.DISPLAY_NAME, cursor.getString(1));
                        values.put(Roster.SORT_KEY, cursor.getString(2));
                    }
                }
            } finally {
                cursor.close();
        ...
        return mDb.update(Tables.ROSTER, values, selection, selectionArgs);
    }

    private int updateRosterData(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        String display_name = null;
        String sort_key = null;
        int count = 0;
         //update Table.Roster_data
        if (values.containsKey(RosterData.DISPLAY_NAME)) {
            display_name = (String)values.get(RosterData.DISPLAY_NAME);
        ...
        if (values.containsKey(RosterData.STATUS))
            Log.e(TAG, "status :" + values.get(RosterData.STATUS) + " " + selection);
        if (values.size() > 0)
            count = mDb.update(Tables.ROSTER_DATA, values, selection, selectionArgs);
        //update Table.Roster(DISPLAY_NAME, DURATION, STATUS)
        int roster_id = 0;
        Cursor c = mDb.query(Tables.ROSTER_DATA, new String[]{RosterData.ROSTER_ID}, selection, selectionArgs, null, null, null);
        try {
            while (c.moveToNext()) {
                roster_id = c.getInt(0);
            }
        } finally {
            c.close();
            c = null;
        ...
        if (roster_id != 0) {
            ContentValues cv = new ContentValues();
            if (display_name != null) {
                cv.put(Roster.DISPLAY_NAME, display_name);
                if (sort_key != null) {
                    cv.put(Roster.SORT_KEY, sort_key);
                }
            }
            if (values.containsKey(RosterData.DURATION)) {
                cv.put(Roster.DURATION, (Integer)values.get(RosterData.DURATION));
            }
            if (cv.size() > 0) {
                mDb.update(Tables.ROSTER, cv, Roster._ID + "=" + roster_id, null);
            }
            cv = null;
        }
    ...
    private int updateContactOptions(long contactId, ContentValues values) {
        ...
        ContactsDatabaseHelper.copyLongValue(mValues, RawContacts.CONTACT_TYPE,
                values, Contacts.CONTACT_TYPE);
        ContactsDatabaseHelper.copyLongValue(mValues, RawContacts.YL_REMARK,
                values, Contacts.YL_REMARK);
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case CONTACTS_STREQUENT: {
                ...
                final String starredQuery = qb.buildQuery(starredProjection, selection/*Contacts.STARRED + "=1"*/,
            ...
            case ROSTER: {
                qb.setTables(Tables.ROSTER);
                qb.setProjectionMap(sRosterProjectionMap);
                break;
            }

            case ROSTER_DATA: {
                qb.setTables(Tables.ROSTER_DATA);
                qb.setProjectionMap(sRosterdataProjectionMap);
                break;
            }
            ...
            case ROSTER_FILTER: {
                ...
                filterParam = Tables.ROSTER + " WHERE " + Roster.SORT_KEY + " LIKE '%" + filterParam + "%'";
                qb.setTables(filterParam);
                qb.setProjectionMap(sRosterProjectionMap);
                break;
            }
            default:
    ...
    private void setTablesAndProjectionMapForContactsWithSnippet(SQLiteQueryBuilder qb, Uri uri,
        ...
        sb.append(
                ...
                " WHERE ((" + NameLookupColumns.NORMALIZED_NAME + " GLOB '");
        sb.append(NameNormalizer.normalize(filter));
        sb.append("*') OR display_name LIKE '%" + filter + "%') AND " + NameLookupColumns.NAME_TYPE +
                    " IN(" + CONTACT_LOOKUP_NAME_TYPES + ")" +
                " GROUP BY " + RawContactsColumns.CONCRETE_CONTACT_ID);
        ...
        switch (match) {
	     ...
	     case CONTACTS_ID_DISPLAY_PHOTO: {
	      ...
	      case RAW_CONTACTS_ID_DISPLAY_PHOTO: {
                ...
                SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
                String[] projection = new String[]{Data._ID, DisplayPhoto.PHOTO_FILE_ID};
                setTablesAndProjectionMapForData(qb, uri, projection, false);
                long photoMimetypeId = mDbHelper.getMimeTypeId(DisplayPhoto.CONTENT_ITEM_TYPE);
    ...
    private AssetFileDescriptor openDisplayPhotoForRead(long photoFileId)
            throws FileNotFoundException {
        PhotoStore.Entry entry = mContactsPhotoStore.get(photoFileId);
        if (entry != null) {
            try {
   ...
   /* private class PipeMonitor extends AsyncTask<Object, Object, Object> {
    ...
    public String getType(Uri uri) {
        ...
        switch (match) {
            ...
            case ROSTER_DATA:
                return RosterData.CONTENT_TYPE;
            case ROSTER_ID:

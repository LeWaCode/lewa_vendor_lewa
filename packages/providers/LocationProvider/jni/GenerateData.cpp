...
inline StringNode * FindString(StringNode * st, const char * str ,const short cardtype)
{
	for(StringNode * ps=st; ps!=NULL; ps=ps->next)
	{
		//cityCode replase cardtype
		if(strcmp(ps->value,str)==0 && ps->cityCode == cardtype)
			return ps;
	}
	return NULL;
}

int MpDataReConvert(const char * fnin, const char * fnout)
{
	...
	for(num = 0 ; num < 2000000; num ++ ) {
		...
		getLocationInfo((char *)fnin,num,location,citycode);
		if(strlen(location) >=  2) {
			fprintf(fpout,"%d,%s,%d\n",num,location,yunyin_code);
			if(num % 1000 == 0)
				printf("get num = %d \n",num);
		}
	}
...
int MpDataConvert(const char * fnin, const char * fnout)
{
	...
	while(!feof(fpin))
	{
		...
		else
		{
			...
			p->NumEnd=numLast;
			//开始新记录
			StringNode * s=FindString(stringTable,addrRead,cityCode);
			if(s==NULL)
			{
...
int main(int argc, char * argv[])
{
	...
	if(argc>1)
	{
		...
		case 'd':
			//导入数据
			ret = MpDataReConvert(val,ChangeFileExt(val,"ref"));
			break;
		case 'h':
			//帮助信息
			printHelp();

...
public class MmsSmsProvider extends ContentProvider {
    ...
    //zhangwei-yl
    private static final int URI_YL_PDU_ALL = 18; 
    private static final int URI_YL_PDU = 19; 	
    private static final int URI_YL_PDU_INBOX = 20;    
    private static final int URI_YL_PDU_OUTBOX = 21;    
    private static final int URI_YL_PDU_MESSAGE = 22;    
    private static final int URI_YL_PDU_CONVERSATIONS_MESSAGE = 23;    
    private static final int URI_YL_PDU_CONVERSATIONS = 24;    
    private static final int URI_YL_PDU_SEARCH = 25;    	
     //zhangwei-yl
    ...
    private static final String TABLE_PDU_YL = "pdu_yl";
    ...
    //zhangwei-yl	
    private static final String[] IMS_ONLY_COLUMNS =
            { "m_id", "person", "address", "contacts_id","status","msg_box",
              "body", "subject", "seen", "m_type", "error_code", "m_data_type",
              "m_size", "data", "path", "file_name"};
    //zhangwei-yl	
    ...
    //zhangwei-yl	
    private static final Set<String> IMS_COLUMNS = new HashSet<String>();
    //zhangwei-yl	
    ...
    //zhangwei-yl
    private static final String IMS_CONVERSATION_CONSTRAINT = "(msg_box != 3)";
    //zhangwei-yl
	...
	//zhangwei-yl
	URI_MATCHER.addURI(AUTHORITY, null, URI_YL_PDU_ALL);
	URI_MATCHER.addURI(AUTHORITY, "pdu_yl", URI_YL_PDU);
       URI_MATCHER.addURI(AUTHORITY, "pdu_yl_inbox", URI_YL_PDU_INBOX);
	URI_MATCHER.addURI(AUTHORITY, "pdu_yl_outbox", URI_YL_PDU_OUTBOX);
	URI_MATCHER.addURI(AUTHORITY, "pdu_yl/#", URI_YL_PDU_MESSAGE);
	URI_MATCHER.addURI(AUTHORITY, "conversations/pdu_yl/#", URI_YL_PDU_CONVERSATIONS_MESSAGE);
	URI_MATCHER.addURI(AUTHORITY, "conversations/pdu_yl", URI_YL_PDU_CONVERSATIONS);	
	URI_MATCHER.addURI(AUTHORITY, "pdu_yl/search", URI_YL_PDU_SEARCH);
	//zhangwei-yl
        ...
        switch(URI_MATCHER.match(uri)) {
	     ...
	     //zhangwei-yl
            case URI_YL_PDU_SEARCH: {
                ...
                String imsProjection = "pdu_yl._id, thread_id,address,body,date, " +
                		" NULL AS index_text, NULL AS _id";
		  ...
		  String imsQuery = String.format(
                        "SELECT %s FROM pdu_yl WHERE (body LIKE ? ) ",
                        imsProjection);		

                // join the results from sms and part (mms)
                String rawQuery = String.format(
                        "%s UNION %s UNION %s GROUP BY %s ORDER BY %s",
                        smsQuery,
                        mmsQuery,
                        imsQuery,
                        "thread_id",
		  ...
		  Log.e(LOG_TAG, "URI_YL_PDU_SEARCH: rawQuery=" + rawQuery);
		  Log.e(LOG_TAG, "URI_YL_PDU_SEARCH: searchString=" + searchString);
	     ...
	     case URI_YL_PDU_CONVERSATIONS:
                 cursor = getYlSimpleConversations(projection, selection, selectionArgs, sortOrder);	
		 break;
		 
	     case URI_YL_PDU_CONVERSATIONS_MESSAGE:

		cursor = getLyConversationMessages(
			uri.getLastPathSegment(), projection, selection,
			selectionArgs, sortOrder);	
		break;
	     case URI_YL_PDU:

		 	cursor = getLyTableMessages(projection, selection, selectionArgs, sortOrder);
		break;	
	     //zhangwei-yl
    ...
    //zhangwei-yl
    private Cursor getYlSimpleConversations(String[] projection, String selection,
            String[] selectionArgs, String sortOrder) {

        return mOpenHelper.getReadableDatabase().query("threads", projection,
                selection, selectionArgs, null, null, " max_date DESC");
    }    
    //zhangwei-yl
    ...
    private Cursor getFirstLockedMessage(String[] projection, String selection,
        ...
        SQLiteQueryBuilder smsQueryBuilder = new SQLiteQueryBuilder();
	//zhangwei-yl	
        SQLiteQueryBuilder imsQueryBuilder = new SQLiteQueryBuilder();

        mmsQueryBuilder.setTables(MmsProvider.TABLE_PDU);
        smsQueryBuilder.setTables(SmsProvider.TABLE_SMS);
        imsQueryBuilder.setTables("pdu_yl");
	//zhangwei-yl
	...
	//zhangwei-yl
	String imsSubQuery = imsQueryBuilder.buildUnionSubQuery(
                MmsSms.TYPE_DISCRIMINATOR_COLUMN, idColumn,
                null, 1, "pdu_yl",
                selection, selectionArgs,
                BaseColumns._ID, "locked=1");
	...
	//zhangwei-yl
        String unionQuery = unionQueryBuilder.buildUnionQuery(
                new String[] { mmsSubQuery, smsSubQuery , imsSubQuery}, null, "1");
	//zhangwei-yl
     ...
     */
	//zhangwei-yl
       private Cursor getLyTableMessages(
      		String[] projection, String selection,
		String[] selectionArgs, String sortOrder) {
		
		return mOpenHelper.getReadableDatabase().query(TABLE_PDU_YL, projection,
                selection, selectionArgs, null, null, " date DESC");
	}
	
	private Cursor getLyConversationMessages(
		...
		try {
			...
			Log.e(LOG_TAG, " getLyConversationMessages () Thread ID must be a Long.");
			return null;
		}

		String finalSelection = concatSelections(
				selection, "thread_id = " + threadIdString);
		String unionQuery = buildYlConversationQuery(
    ...
    //zhangwei-yl
    private static String[] createYlProjection(String[] old) {
        String[] newProjection = new String[old.length];
        for (int i = 0; i < old.length; i++) {
                newProjection[i] = old[i];
        }
    ...
    private static String buildYlConversationQuery(String[] projection,
            String selection, String[] selectionArgs, String sortOrder) {
        String[] mmsProjection = createMmsProjection(projection);
        String[] ylProjection = createYlProjection(projection);

        SQLiteQueryBuilder mmsQueryBuilder = new SQLiteQueryBuilder();
        SQLiteQueryBuilder smsQueryBuilder = new SQLiteQueryBuilder();
        SQLiteQueryBuilder ylQueryBuilder = new SQLiteQueryBuilder();

        mmsQueryBuilder.setDistinct(true);
        smsQueryBuilder.setDistinct(true);
        ylQueryBuilder.setDistinct(true);
		
        mmsQueryBuilder.setTables(joinPduAndPendingMsgTables());
        smsQueryBuilder.setTables(SmsProvider.TABLE_SMS);
        ylQueryBuilder.setTables(TABLE_PDU_YL);

        String[] smsColumns = handleNullMessageProjection(projection);
        String[] ylColumns = handleNullMessageProjection(ylProjection);
        String[] mmsColumns = handleNullMessageProjection(mmsProjection);
        ...
        String[] innerylProjection = makeProjectionWithNormalizedDate(ylColumns, 1);
        ...
        String smsSubQuery = smsQueryBuilder.buildUnionSubQuery(
                ...
                selectionArgs, null, null);		
        String ylSubQuery = ylQueryBuilder.buildUnionSubQuery(
                MmsSms.TYPE_DISCRIMINATOR_COLUMN, innerylProjection, IMS_COLUMNS,
                0, "ims", concatSelections(selection, IMS_CONVERSATION_CONSTRAINT),
                selectionArgs, null, null);
        ...
        String unionQuery = unionQueryBuilder.buildUnionQuery(
                new String[] { smsSubQuery, ylSubQuery, mmsSubQuery },
    ...
    public int delete(Uri uri, String selection,
        ...
        switch(URI_MATCHER.match(uri)) {
            ...
            case URI_OBSOLETE_THREADS:
                ...
                //zhangwei-yl        
                        "UNION SELECT DISTINCT thread_id FROM pdu_yl " +
                //zhangwei-yl        
		...
		affectedRows = MmsProvider.deleteMessages(context, db,
				selection, selectionArgs, uri)
				+ db.delete("sms", selection, selectionArgs)
				+ db.delete(TABLE_PDU_YL, selection, selectionArgs);
		MmsSmsDatabaseHelper.updateAllYlThreads(db, selection, selectionArgs);
		MmsSmsDatabaseHelper.updateAllThreads(db, null, null);
		break;	
	     case URI_YL_PDU_CONVERSATIONS_MESSAGE:
                long thread_Id;
		  ...
		  affectedRows = deleteYlConversation(uri, selection, selectionArgs);
		  MmsSmsDatabaseHelper.updateThread(db, thread_Id);
		  MmsSmsDatabaseHelper.updateImsThread(db, thread_Id);
		 break;
	     case URI_YL_PDU_MESSAGE:
	 	int message_id;
		
		try {
			message_id = Integer.parseInt(uri.getLastPathSegment());
		} catch (NumberFormatException e) {
			Log.e(LOG_TAG, "message id must be a int.");
			break;
		}
	 	
	 	affectedRows = MmsSmsDatabaseHelper.deleteOneYlMessage(db, message_id);
		 break;
	     //zhangwei-yl
    ...
    //zhangwei-yl
    private int deleteYlConversation(Uri uri, String selection, String[] selectionArgs) {
        String threadId = uri.getLastPathSegment();
        ...
        return MmsProvider.deleteMessages(getContext(), db, finalSelection,
                                          selectionArgs, uri)
                + db.delete("sms", finalSelection, selectionArgs)
                + db.delete(TABLE_PDU_YL, finalSelection, selectionArgs);
    }
    //zhangwei-yl
    @Override
	...
	String table = TABLE_PDU_YL;
	int type = URI_YL_PDU_ALL;	
	
	switch (match) {
		case URI_YL_PDU_INBOX:
			type = Sms.MESSAGE_TYPE_INBOX;
			break;	
			
            	case URI_YL_PDU_OUTBOX:
	...
	if (table.equals(TABLE_PDU_YL)){

		boolean addDate = false;
		boolean addType = false;

		if (values == null){
			inputValues = new ContentValues(1);
			addDate = true;
		}else{
			inputValues = new ContentValues(values);

			if (!values.containsKey("date")) {
				addDate = true;
			}

			if (!values.containsKey("msg_box")) {
				addType = true;
			}
		}

		if (addDate) {
			inputValues.put("date", new Long(System.currentTimeMillis()));
		}

		if (addType && (type != URI_YL_PDU_ALL)) {
			inputValues.put("msg_box", Integer.valueOf(type));
		}
		...
		if (((threadId == null) || (threadId == 0)) && (address != null)) {
			inputValues.put("thread_id", Threads.getOrCreateThreadId(getContext(), address));
		}

		if (type == Sms.MESSAGE_TYPE_INBOX){
			inputValues.put("status",  Integer.valueOf(Sms.STATUS_NONE));			
		}else{
			inputValues.put("read",  Integer.valueOf(1));
			inputValues.put("seen",  Integer.valueOf(1));
			inputValues.put("status",  Integer.valueOf(Sms.STATUS_PENDING));			
		}
		
		rowID = db.insert(table, null, inputValues);		
	}
		
	if (rowID > 0) {
		Uri retUri = Uri.parse("content://" + table + "/" + rowID);
		getContext().getContentResolver().notifyChange(MmsSms.CONTENT_URI, null);
		return retUri;
	} else {
		Log.e("MmsSmsProvider" ,"insert: failed! " + values.toString());
	}
    ...
    public int update(Uri uri, ContentValues values,
        ...
        switch(URI_MATCHER.match(uri)) {
	     ...
	     //zhangwei-yl
	     case URI_YL_PDU:		 
		 affectedRows = db.update(TABLE_PDU_YL, values, selection, selectionArgs);
		 MmsSmsDatabaseHelper.updateAllYlThreads(db, selection, selectionArgs);
		 break;
	     //zhangwei-yl
	...
	//zhangwei-yl
	int imsOnlyColumnCount = IMS_ONLY_COLUMNS.length;
	//zhangwei-yl
        ...
        for (int i = 0; i < commonColumnCount; i++) {
	     ...
	     //zhangwei-yl
	     IMS_COLUMNS.add(MMS_SMS_COLUMNS[i]);
	     //zhangwei-yl
	...
	//zhangwei-yl	
        for (int i = 0; i < imsOnlyColumnCount; i++) {
            IMS_COLUMNS.add(IMS_ONLY_COLUMNS[i]);
            unionColumns.add(IMS_ONLY_COLUMNS[i]);
        }
	 //zhangwei-yl	
        int i = 0;

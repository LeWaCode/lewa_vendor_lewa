...
public class MmsSmsDatabaseHelper extends SQLiteOpenHelper {
    ...
    //zhangwei-yl
    private static final String PDU_YL_UPDATE_THREAD_READ_BODY =
                        "  UPDATE threads SET yl_read = " +
                        "    CASE (SELECT COUNT(*)" +
                        "          FROM pdu_yl" +
                        "          WHERE " + "read" + " = 0" +
    ...
    //zhangwei-yl	
    private static final String PDU_YL_UPDATE_THREAD_COUNT_ON_NEW =
                        "  UPDATE threads SET yl_count = " +
                        "     (SELECT COUNT(pdu_yl._id) FROM pdu_yl LEFT JOIN threads " +
   ...
   //zhangwei-yl
   private static final String PDU_YL_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE =
					   "BEGIN" +
					   "  UPDATE threads SET" +
					   "	yl_date = (strftime('%s','now') * 1000), " +
					   "	yl_snippet = new." + "body, " + 
					   "   yl_data_type = new.m_data_type " +                      
					   "   WHERE threads._id = new." + "thread_id" + "; " +
					   PDU_YL_UPDATE_THREAD_COUNT_ON_NEW +
					   PDU_YL_UPDATE_THREAD_READ_BODY +
					   "END;";
   //zhangwei-yl
    ...
    //zhangwei-yl
    public static void updateImsThread(SQLiteDatabase db, long thread_id){

	int rows = db.delete("threads",
		"_id = ? AND _id NOT IN" +
		"          (SELECT thread_id FROM sms " +
		" UNION SELECT  thread_id FROM pdu_yl " +
		"           UNION SELECT thread_id FROM pdu)",
		new String[] { String.valueOf(thread_id) });

	Log.e(TAG, "updateImsThread rows= " + rows);
	Log.e(TAG, "updateImsThread thread_id= " + thread_id);
        if (rows > 0) {
        ...
        db.execSQL(
            ...
            "  yl_date =" +
            "  (SELECT date FROM" +
            "  (SELECT date, thread_id FROM pdu_yl)" +
            "  WHERE thread_id = " + thread_id + " ORDER BY date DESC LIMIT 1)," +
            "  yl_data_type =" +
            "  (SELECT m_data_type FROM" +
            "  (SELECT date, m_data_type, thread_id FROM pdu_yl)" +
            "  WHERE thread_id = " + thread_id + " ORDER BY date DESC LIMIT 1)," +
            "  yl_snippet =" +
            "  (SELECT snippet FROM" +
            "  (SELECT date, body AS snippet, thread_id FROM pdu_yl)" +
            "  WHERE thread_id = " + thread_id + " ORDER BY date DESC LIMIT 1)" +
            "  WHERE threads._id = " + thread_id + ";");

        String query = "SELECT thread_id FROM pdu_yl WHERE msg_box=" +
        ...
        String errorQuery = "SELECT yl_error FROM threads WHERE _id = " + thread_id;
            ...
            try {
                if (c.moveToNext()) {
                    ...
                    if (curError != setError) {
                        ...
                        db.execSQL("UPDATE threads SET yl_error=" + setError +
                                " WHERE _id = " + thread_id);
                    }
                }
            } finally {
    ...
    //zhangwei-yl
    public static void updateAllYlThreads(SQLiteDatabase db, String where, String[] whereArgs) {
        if (where == null) {
            where = "";
        } else {
            where = "WHERE (" + where + ")";
        ...
        String query = "SELECT _id FROM threads WHERE _id IN " +
                       "(SELECT DISTINCT thread_id FROM pdu_yl " + where + ")";
        Cursor c = db.rawQuery(query, whereArgs);
        if (c != null) {
            try {
                while (c.moveToNext()) {
		      Log.e(TAG, "updateAllYlThreads thread= " + c.getInt(0));		
                    updateImsThread(db, c.getInt(0));
                }
            } finally {
                c.close();
            }
    ...
    public static int deleteOneYlMessage(SQLiteDatabase db, int message_id) {
        int thread_id = -1;
        // Find the thread ID that the specified SMS belongs to.
        Cursor c = db.query("pdu_yl", new String[] { "thread_id" },
        ...
        int rows = db.delete("pdu_yl", "_id=" + message_id, null);
        if (thread_id > 0) {
            // Update its thread.
            updateImsThread(db, thread_id);			
        }
        return rows;
    }
    //zhangwei-yl
    ...
    private void createSmsTables(SQLiteDatabase db) {
	 ...
	 //zhangwei-yl
        db.execSQL("CREATE TABLE pdu_yl (" +
                   "_id INTEGER PRIMARY KEY," +
                   "thread_id INTEGER," +
                   "m_id INTEGER," +
                   "person TEXT," +
                   "address TEXT," +
                   "contacts_id INTEGER DEFAULT 0," +                   
                   "date INTEGER," +
                   "read INTEGER DEFAULT 0," +
                   "status INTEGER," +                   
                   "msg_box INTEGER," +                   
                   ...
                   "m_data_type TEXT," +
                   "m_size INTEGER DEFAULT 0," +
                   "data TEXT," +
                   "path TEXT," +
                   "file_name TEXT" +
                   ");");	

        db.execSQL("CREATE TABLE statistics_yl (" +
                   "_id INTEGER PRIMARY KEY," +
                   "address TEXT," +
                   "date INTEGER," +
                   "status INTEGER," +                   
    ...
    private void createCommonTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE threads (" +
                   ...
                   "yl_name TEXT," +
		     "yl_date INTEGER DEFAULT 0," +
	            "yl_snippet TEXT," +
	            "yl_read INTEGER DEFAULT 1," +
	            "yl_error INTEGER DEFAULT 0," +
	            "yl_count INTEGER DEFAULT 0," +	    
                   "yl_has_attachment INTEGER DEFAULT 0,"+
                   "yl_data_type TEXT );");
		     //zhangwei-yl
	...
	//zhangwei-yl
	db.execSQL("CREATE TRIGGER pdu_yl_update_thread_on_insert AFTER INSERT ON pdu_yl " +
			   PDU_YL_UPDATE_THREAD_DATE_SNIPPET_COUNT_ON_UPDATE);
	//zhangwei-yl
	...
	//zhangwei-yl
	db.execSQL("CREATE TRIGGER pdu_ly_update_thread_read_on_update AFTER" +
			   "  UPDATE OF " + " read " +
			   "  ON pdu_yl " +
			   "BEGIN " +
			   PDU_YL_UPDATE_THREAD_READ_BODY +
	...
	//zhangwei-yl
	db.execSQL("DROP TABLE IF EXISTS pdu_yl");
	//zhangwei-yl		
	...
	try{
		//add threads column by yl
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_name  TEXT");
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_date  INTEGER DEFAULT 0");
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_snippet  TEXT");
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_read  INTEGER DEFAULT 1");
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_error  INTEGER DEFAULT 0");
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_count  INTEGER DEFAULT 0");			
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_has_attachment  INTEGER DEFAULT 0");
		
		//add table pdu_yl by yl
		db.execSQL("CREATE TABLE pdu_yl (" +
                   "_id INTEGER PRIMARY KEY," +
                   "thread_id INTEGER," +
                   "m_id INTEGER," +
		...
		//delete all trigger
		dropAllCommonTriggers(db);
		//add trigger
		createCommonTriggers(db);		
	}catch(Exception ex){
		Log.e(TAG, "[MmsSmsDb] upgradeDatabaseToVersion54 caught ", ex);
	}
    }
	...
	try{
		//add threads column by yl
		db.execSQL("ALTER TABLE threads ADD COLUMN yl_data_type  TEXT");	
		...
		Log.e(TAG, "[MmsSmsDb] upgradeDatabaseToVersion55 caught ", ex);
	}
    }
    

    private void dropAllCommonTriggers(SQLiteDatabase db){
	...
	db.execSQL("DROP TRIGGER IF EXISTS pdu_yl_update_thread_on_insert");		
	...
	db.execSQL("DROP TRIGGER IF EXISTS pdu_ly_update_thread_read_on_update");

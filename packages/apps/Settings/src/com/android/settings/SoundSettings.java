...
//liuhao fix huawei u8800 special haptic  20111219
...
public class SoundSettings extends PreferenceActivity implements
    ...
    //Begin [Added for bug 3560, 20120302]
    ...
    public static final String VIBRATE_CONF = "/system/misc/30vibrate.conf";
    ...
    /*deleted by chenqiang for bug 5447,20120417
    private CheckBoxPreference mNotifySpeaker;
    private CheckBoxPreference mRingSpeaker;
    private CheckBoxPreference mAlarmSpeaker;
    */
    private CheckBoxPreference mSilenceRingEvertPhone;
   ...
   //[Begin for lockscreen sound,add fulianwu 20111228]	
    private static final String LOCKSCREEN_CUSTOM_SOUND_TOGGLE = "pref_lockscreen_sound_toggle";	
    private CheckBoxPreference mLockscreenCustomSoundToggle;	
    //[End]
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSilenceRingEvertPhone = (CheckBoxPreference) findPreference(KEY_SILENCE_RING_EVERT_PHONE);
        // Begin, fixed the bug 7558, 20120626
        if ("n880s".equalsIgnoreCase(Build.DEVICE)) {
            getPreferenceScreen().removePreference(mSilenceRingEvertPhone);
        } else {            
            String silenceRingEvertPhone =  Settings.System.getString(getContentResolver(),
            ...
            mSilenceRingEvertPhone.setChecked(silenceRingEvertPhoneEnabled);
            Settings.System.putString(getContentResolver(),KEY_SILENCE_RING_EVERT_PHONE, String.valueOf(silenceRingEvertPhoneEnabled));
        }
        // End
        ...
        if (amberGreenLight) {
         ...
         /*deleted by chenqiang for bug 5447,20120417
        mRingSpeaker = (CheckBoxPreference) findPreference(RINGS_SPEAKER);
        mRingSpeaker.setChecked(SystemProperties.getBoolean(getKey(RINGS_SPEAKER), false));
        mRingSpeaker.setOnPreferenceChangeListener(this);

        mAlarmSpeaker = (CheckBoxPreference) findPreference(ALARMS_SPEAKER);
        mAlarmSpeaker.setChecked(SystemProperties.getBoolean(getKey(ALARMS_SPEAKER), false));
        mAlarmSpeaker.setOnPreferenceChangeListener(this);

        mNotifySpeaker = (CheckBoxPreference) findPreference(NOTIFICATIONS_SPEAKER);
        mNotifySpeaker.setChecked(SystemProperties.getBoolean(getKey(NOTIFICATIONS_SPEAKER), false));
        mNotifySpeaker.setOnPreferenceChangeListener(this);
        */
        //[Begin for lockscreen sound,add fulianwu 20111228]
		
        PreferenceScreen prefSet = getPreferenceScreen();
		
        //0 is off,1 is on,and default value is 1		
        mLockscreenCustomSoundToggle = (CheckBoxPreference) prefSet.findPreference(LOCKSCREEN_CUSTOM_SOUND_TOGGLE);        
        mLockscreenCustomSoundToggle.setChecked(Settings.System.getInt(getContentResolver(),
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mSilent) {
            ...
            //liuhao fix huawei u8800 special haptic  20111219			                   
            setHaptic_Hardware(mHapticFeedback.isChecked() );
        ...
        } else if (preference == mSilenceRingEvertPhone ) {
            boolean value = mSilenceRingEvertPhone.isChecked();
            Settings.System.putString(getContentResolver(),
                    KEY_SILENCE_RING_EVERT_PHONE, Boolean.toString(value));
        } else if(preference == mLockscreenCustomSoundToggle){			
        	boolean value = mLockscreenCustomSoundToggle.isChecked();            
			Settings.System.putInt(getContentResolver(),
					Settings.System.LOCKSCREEN_SOUND_SWITCH, value ? 1 : 0);
        } 
        return true;
    }
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        }*/ //deleted by chenqiang for bug 5447

        return true;
    }

    //liuhao fix huawei u8800 special haptic  20111219
   public static void setHaptic_Hardware(boolean check_value) {
	String productVersion = SystemProperties.get("ro.product.model");
 	String value = check_value ? "30" : "0";     
        boolean WRITE = false;                 
    	if(0 == productVersion.compareToIgnoreCase("U8800")) {
            writeOneLine(HAPTIC_FILE, value);
            writeOneLine(HAPTIC_FILE1, value);
            WRITE = runRootCommand("mount -o rw,remount -t ext3 /dev/block/mmcblk0p12 /system");
            if (WRITE) {
    		  runRootCommand("echo "+ "VIBRATE_VALUE=" + value + " > " + VIBRATE_CONF);
    		  runRootCommand("chmod +x "+ VIBRATE_CONF);
    		  runRootCommand("mount -o ro,remount -t ext3 /dev/block/mmcblk0p12 /system");
            } else {
                Log.e(TAG, "can 't get root command");
            }
        }
    }

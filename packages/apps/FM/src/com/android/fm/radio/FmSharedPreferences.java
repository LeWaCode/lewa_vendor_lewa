...
public class FmSharedPreferences {
    ...
    // by chenhengheng default china feq
    // modify maowenjiang change DEFAULT = china
    ...
    private static final String AUTOSEARCHSTATIONS = "auto_stations";
    ...
    private static final int DEFAULT_NO_FREQUENCY = 106500;
    ...
    // modify maowenjiang mCountry = 8 2012-02-29
    private static int mCountry = 8;
    ...
    private static String mAutoSearchStations = "";
    ...
    public static void setStationName(int listIndex, int stationIndex,
            String name) {
        if (listIndex < getNumList()) {
    ...
    public static void setStationFreq(int listIndex, int stationIndex, int freq) {
        // modify by even 2012-03-07 'add listIndex < mListOfPlists.size()'
        if (listIndex < mListOfPlists.size() && listIndex < getNumList()) {
            mListOfPlists.get(listIndex).setStationFreq(stationIndex, freq);
        }
    }

    public static void setStation(int listIndex, int stationIndex, String name,
            int freq) {
        // modify by even 2012-03-07 'add listIndex < mListOfPlists.size()'
        if (listIndex < mListOfPlists.size() && listIndex < getNumList()) {
            mListOfPlists.get(listIndex).setStationName(stationIndex, name);
    ...
    public static double getStationFrequency(int listIndex, int stationIndex) {
        double frequency = 0;
        if (listIndex < getNumList()) {
            frequency = mListOfPlists.get(listIndex).getStationFrequency(
                    stationIndex);
        }
        return frequency;
    }
    ...
    public static PresetStation getStationFromFrequency(int frequency) {
        ...
        PresetStation station = null;
        if (listIndex < getNumList()) {
            station = mListOfPlists.get(listIndex).getStationFromFrequency(
                    frequency);
        }
        return station;
    }

    public static int getIndexFromStation(PresetStation station) {
        int listIndex = getCurrentListIndex();
        return mListOfPlists.get(listIndex).getIndexFromStation(station);
    }
    ...
    public void Load() {
        ...
        SharedPreferences sp = mContext.getSharedPreferences(SHARED_PREFS,
                Context.MODE_PRIVATE);
        mTunedFrequency = sp.getInt(PREF_LAST_TUNED_FREQUENCY,
        ...
        for (int listIter = 0; listIter < num_lists; listIter++) {
            ...
            for (int stationIter = 0; stationIter < numStations; stationIter++) {
                String stationName = sp.getString(STATION_NAME + listIter + "x"
                        + stationIter, DEFAULT_NO_NAME);
                int stationFreq = sp.getInt(STATION_FREQUENCY + listIter + "x"
                        + stationIter, DEFAULT_NO_FREQUENCY);
                PresetStation station = curList.addStation(stationName,
                        stationFreq);

                int stationId = sp.getInt(STATION_ID + listIter + "x"
                        + stationIter, DEFAULT_NO_STATIONID);
                station.setPI(stationId);

                int pty = sp.getInt(STATION_PTY + listIter + "x" + stationIter,
                        DEFAULT_NO_PTY);
                station.setPty(pty);

                int rdsSupported = sp.getInt(STATION_RDS + listIter + "x"
                        + stationIter, DEFAULT_NO_RDSSUP);
                if (rdsSupported != 0) {
        ...
        setAutoSearchStations(sp.getString(AUTOSEARCHSTATIONS, ""));
        ...
        // modify by Even 2012-03-14
        setBluetoothExitBehaviour(1);
        setHeadsetDcBehaviour(sp
                .getBoolean(Settings.HEADSET_DC_BEHAVIOUR, true));
    ...
    public void Save() {
        ...
        for (int listIter = 0; listIter < numLists; listIter++) {
            ...
            int numStation = 0;
            for (int stationIter = 0; stationIter < numStations; stationIter++) {
                PresetStation station = curList
                        .getStationFromIndex(stationIter);
                if (station != null) {
        ...
        ed.putString(AUTOSEARCHSTATIONS, mAutoSearchStations);
        ed.commit();
    }
    ...
    /**
     * @param mFMConfiguration
     *            the mFMConfiguration to set
     */
    ...
    public static void setRadioBand(int band) {
        ...
        // modify by maowenjiang 2012-03-06
    ...
    public static void setCountry(int nCountryCode) {
        ...
        // Default: 87000 TO 108000 IN 100 KHZ STEPS modify by maowenjiang
        // 2012-02-29
    ...
    public static void setAutoSearchStations(String stations) {
        mAutoSearchStations = stations;
    }

    public static String getAutoSearchStations() {
        return mAutoSearchStations;
    }

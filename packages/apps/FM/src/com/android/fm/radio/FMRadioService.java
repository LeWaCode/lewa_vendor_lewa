...
public class FMRadioService extends Service {
    ...
    /**
     * Resource to read Radio Data System (RDS) information embedded in a radio
     * stream if it exists
     */
    ...
    private final Handler mNotificationHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            case UPDATE_STAT_NOTIFICATION:
                if (isFmOn()) {
                    statusBarViews.setTextViewText(R.id.frequency,
                            getTunedFrequencyString());
                } else {
                    statusBarViews.setTextViewText(R.id.frequency,
                            getString(R.string.stat_no_fm));
                }
    ...
    public void onCreate() {
        ...
        // Register our MediaButtonEventReceiver so it receives the lockscreen
        // events
        // Instantiate a handle to our shared preferences
        // TODO: These should be stored in a properties file and read from via a
        // ResourceBundle
        ...
        // modify by even 2012-03-09 resolve service permannet
        mDelayedStopHandler.sendMessageDelayed(msg, IDLE_DELAY);

        statusBarViews = new RemoteViews(getPackageName(), R.layout.statusbar);
        statusBarViews.setImageViewResource(R.id.icon,
                R.drawable.stat_notify_fm);
        ...
        statusBarNotification.contentIntent = PendingIntent.getActivity(this,
                0, new Intent(this, FMRadio.class), 0);
    ...
    public void onDestroy() {
        ...
        if (LOG_STATUS) {
            Log.v(LOGTAG, "onDestroy: Giving up audio focus");
        }
        ...
        fmOff();
        if (LOG_STATUS) {
            Log.v(LOGTAG, "onDestroy: unbindFromService completed");
        }
    ...
    private OnAudioFocusChangeListener mAudioFocusListener = new OnAudioFocusChangeListener() {
        public void onAudioFocusChange(int focusChange) {
            ...
            case AudioManager.AUDIOFOCUS_LOSS:
                if (LOG_STATUS) {
                    Log.v(LOGTAG,
                            "AudioFocus: received AUDIOFOCUS_LOSS, turning FM off");
                }
                if (isFmOn()) {
                    fmOff();
                }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (LOG_STATUS) {
            Log.v(LOGTAG, "service onStartCommand");
        }
        mServiceStartId = startId;
        mDelayedStopHandler.removeCallbacksAndMessages(null);
        String searchStations = mPrefs.getAutoSearchStations();
        if (intent != null) {
            if (LOG_STATUS) {
                Log.d(LOGTAG,
            ...
            // We don't act on Action for Lockscreen controls as the actual
            // command is stored
            ...
            if (FMRadioService.CMDTOGGLEPAUSE.equals(cmd)) {
                ...
                if (isFmOn()) {
                    if (context.getResources()
                            .getBoolean(R.bool.seek_supported)) {
                        /**
                         * added by even 2012-03-26
                         */
                        if (searchStations != null) {
                            String[] searchStationsArray = searchStations
                                    .split(" ");
                            if (searchStationsArray != null
                                    && searchStationsArray.length > 0) {
                                int stationCount = searchStationsArray.length;
                                int currFreq = getFreq();
                                int currFreqIndex = 0;
                                for (int i = 0; i < searchStationsArray.length; i++) {
                                    if (searchStationsArray[i].equals(currFreq
                                            + "")) {
                                        currFreqIndex = i;
                                    }
                                }

                                int tuneFreq = mPrefs.getTunedFrequency();
                                if (currFreqIndex != -1
                                        && currFreqIndex < stationCount - 1) {
                                    tuneFreq = new Integer(
                                            searchStationsArray[currFreqIndex + 1]);
                                } else {
                                    tuneFreq = new Integer(
                                            searchStationsArray[0]);
                                }
                                tune(tuneFreq);
                                mPrefs.setTunedFrequency(tuneFreq);
                                mPrefs.Save();
                            }
                        } else {
                            seek(true);
                    ...
                    if (context.getResources()
                        ...
                        if (searchStations != null) {
                            ...
                            if (searchStationsArray != null
                                ...
                                int tuneFreq = mPrefs.getTunedFrequency();
                                if (currFreqIndex != -1 && currFreqIndex > 0) {
                                    tuneFreq = new Integer(
                                            searchStationsArray[currFreqIndex - 1]);
                                } else {
                                    tuneFreq = new Integer(
                                            searchStationsArray[stationCount - 1]);
                                }
                                tune(tuneFreq);
                                mPrefs.setTunedFrequency(tuneFreq);
                                mPrefs.Save();
    ...
    // Intent i = new Intent("com.android.fm.metachanged");
    // if(on){
    ...
    private void stop() {
        // added by even set speaker false when fm start 2012-05-28
        mPrefs.setSpeaker(false);
        mPrefs.Save();
        switchToHeadset();
        gotoIdleState();
        mFMOn = false;
    ...
    private void updateNotification() {
        ...
        // delay 1000ms so that we won't send notification too frequently during
        // move
        mNotificationHandler.sendEmptyMessageDelayed(UPDATE_STAT_NOTIFICATION,
                1000);
    }
    ...
    private String getTunedFrequencyString() {
        String frequencyString = getString(R.string.stat_notif_frequency,
                FrequencyPicker.formatFrequencyString(FmSharedPreferences
                        .getTunedFrequency()));
    ...
    public void readInternalAntennaAvailable() {
        ...
        if (mReceiver != null) {
            ...
            if (LOG_STATUS) {
                Log.d(LOGTAG, "getInternalAntenna: "
    ...
    private boolean fmOn() {
        if (LOG_STATUS) {
            Log.i(LOGTAG, "service fmOn start");
        }
        ...
        if (mReceiver == null) {
            if (LOG_STATUS) {
                Log.i(LOGTAG, "service fmOn start mReceiver null init");
            }
        ...
        if (mReceiver != null) {
            if (LOG_STATUS) {
                Log.i(LOGTAG, "service fmOn start mReceiver not null ");
            }
            if (isFmOn()) {
                /* FM Is already on, */
                bStatus = true;
                ...
                if (LOG_STATUS) {
                    ...
                    Log.d(LOGTAG,
                            "fmOn: LowerLimit  :" + config.getLowerLimit());
                    Log.d(LOGTAG,
                            "fmOn: UpperLimit  :" + config.getUpperLimit());
                }

                bStatus = mReceiver.enable(FmSharedPreferences
                        .getFMConfiguration());
                if (LOG_STATUS) {
            ...
            if (bStatus == true) {
                startFM();
                bStatus = mReceiver
                        .registerRdsGroupProcessing(FmReceiver.FM_RX_RDS_GRP_RT_EBL
                                | FmReceiver.FM_RX_RDS_GRP_PS_EBL
                                | FmReceiver.FM_RX_RDS_GRP_AF_EBL
                                | FmReceiver.FM_RX_RDS_GRP_PS_SIMPLE_EBL);
                if (LOG_STATUS) {
                    Log.d(LOGTAG, "registerRdsGroupProcessing done, Status :"
                            + bStatus);
                }
    ...
    private boolean fmOff() {
        ...
        if (mReceiver != null) {
            if (LOG_STATUS) {
                Log.i(LOGTAG, "fmOff mReceiver");
            }
            bStatus = mReceiver.disable();
            bStatus = mReceiver.release();
            mReceiver = null;
        }
        if (!mBluetoothEnabled && mBooRequireBluetooth) {
    ...
    public boolean routeAudio(int audioDevice) {
        ...
        if (mReceiver != null) {
            if ((audioDevice == RADIO_AUDIO_DEVICE_WIRED_HEADSET)
                ...
                Log.e(LOGTAG, "routeAudio: Unsupported Audio Device: "
                        + audioDevice);
            }
        }
        return bStatus;
    ...
    public boolean mute() {
        ...
        try {
            ...
            ex.printStackTrace();
            Log.d(LOGTAG, "mute sleep 100ms failure");
        }
        if (mReceiver != null) {
            mMuted = true;
     ...
     * 
     * @param up
     *            increments higher if {code true}, lower if {code false}
    ...
    public boolean nextFrequency(boolean up) {
        ...
        if (mReceiver != null) {
            /*
             * We have to grab the current freq from FmSharedPreferences rather
             * than mReceiver since any changes to freq get saved to
             * FmSharedPreferences and NOT mReceiver
             */
        ...
        if (mReceiver != null) {
            ...
            if (FmSharedPreferences.isRBDSStd()) {
                ...
                if (((pty > 0) && (pty <= 23)) || ((pty >= 29) && (pty <= 31))) {
                    bCommandSent = mReceiver.searchStations(
                            FmReceiver.FM_RX_SRCHRDS_MODE_SCAN_PTY,
    ...
    public boolean searchStrongStationList(int numStations) {
        ...
        if (mReceiver != null) {
            if (LOG_STATUS) {
                Log.d(LOGTAG, "searchStrongStationList:  numStations: "
                        + numStations);
            }
            bCommandSent = mReceiver.searchStationList(
    ...
    public boolean seekPI(int piCode) {
        ...
        if (mReceiver != null) {
            ...
            bCommandSent = mReceiver.searchStations(
                    FmReceiver.FM_RX_SRCHRDS_MODE_SEEK_PI,
                    FmReceiver.FM_RX_DWELL_PERIOD_1S,
                    FmReceiver.FM_RX_SEARCHDIR_UP, 0, piCode);
        }
        return bCommandSent;
    }
        ...
        if (mReceiver != null) {
            ...
            if (bLowPower) {
                bCommandSent = mReceiver
                        .setPowerMode(FmReceiver.FM_RX_LOW_POWER_MODE);
            } else {
                bCommandSent = mReceiver
                        .setPowerMode(FmReceiver.FM_RX_NORMAL_POWER_MODE);
            }
        }
        return bCommandSent;
    }
                    ...
                    if (mFMRxRDSData != null) {
                        if (LOG_STATUS) {
                            ...
                            Log.d(LOGTAG, "PTY: [" + mFMRxRDSData.getPrgmType()
                                    + "]");
                            Log.d(LOGTAG,
                                    "PS: [" + mFMRxRDSData.getPrgmServices()
                                            + "]");
                        }

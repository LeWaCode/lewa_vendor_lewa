...
public class CallNotifier extends Handler
    ...
    //added by yuzhijian , 20120629
    private static final long TIMEINTERCEPT = 3000;
    ...
    private static final int RINGTONE_QUERY_WAIT_TIME =
            ...
            //modify by shenqi for one ring interept
    private static final int RINGTONE_INTEREPT_WAIT_TIME = 1500; // msec
	     //end by shenqi
    ...
    // Event used to indicate a query timeout.
    private static final int RINGER_CUSTOM_RINGTONE_DELAY_TIMEOUT = 101;
    ...
    private static final String KEY_SILENCE_RING_EVERT_PHONE = "silence_ring_evert_phone";
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case PHONE_UNKNOWN_CONNECTION_APPEARED:
                ...
                break;
	     case RINGER_CUSTOM_RINGTONE_DELAY_TIMEOUT:
    ...
    private void onNewRingingConnection(AsyncResult r) {
        ...
        if (android.text.TextUtils.isEmpty(number)) {
            number = "0000";
            PhoneApp.isNumberEmpty = true;
        }else{
//            //start, added by yuzhijian ,20120702
//            DbHelper dbHelper = new DbHelper(newphone.getContext(), "intercept.db", null , 1);
//            SQLiteDatabase IDb = dbHelper.getWritableDatabase();
//            Cursor cursor=IDb.rawQuery( "SELECT * FROM "+InterceptConstants.CALL_TABLENAME , null);
//            if (cursor.getColumnCount()<8) {
//                IDb.execSQL("Alter table " +  InterceptConstants.CALL_TABLENAME + " add cause");
//            }
//            cursor.close();
//            IDb.close();
//            //end
            LewaBlocker lewaBlocker = LewaBlocker.getInstance(newphone.getContext());
            lewaBlocker.block(ringing,newphone,number);
            PhoneApp.isNumberEmpty = false;
        }
        if (DBG) log("incoming number is: " + number);

        // Incoming calls are totally ignored if OTA call is active
        if (TelephonyCapabilities.supportsOtasp(newphone)) {
        ...
        if (newphone.getPhoneType() == Phone.PHONE_TYPE_CDMA) {
            stopSignalInfoTone();
        }
        ...
        if (PhoneUtils.isRealIncomingCall(state)) {
            ...
            String silenceRingEvertPhone =  Settings.System.getString(mContentResolver,
                    KEY_SILENCE_RING_EVERT_PHONE);
            boolean silenceRingEvertPhoneEnabled = false;
            if (silenceRingEvertPhone != null) {
                silenceRingEvertPhoneEnabled = Boolean.valueOf(silenceRingEvertPhone);
            }
            Log.v(LOG_TAG,"silenceRingEvertPhoneEnabled: " + Boolean.toString(silenceRingEvertPhoneEnabled));
            if (silenceRingEvertPhoneEnabled) {
                mSensorManager.registerListener(mEvertListener, mSensor,
                        SensorManager.SENSOR_DELAY_NORMAL);
                mSensorManager.registerListener(mEvertListener, mOritationSensor,
    ...
    private void startIncomingCallQuery(Connection c) {
        ...
        if (shouldStartQuery) {
		 ...
		 //modify for one ring call intererpt by shenqi
                sendEmptyMessageDelayed(RINGER_CUSTOM_RINGTONE_QUERY_TIMEOUT,
                        RINGTONE_QUERY_WAIT_TIME + RINGTONE_INTEREPT_WAIT_TIME);
		 //end by shenqi
            }
    ...
    public void onQueryComplete(int token, Object cookie, CallerInfo ci) {
        if (cookie instanceof Long) {
		 ...
		 // modify for one ring intercrept by shenqi
		  sendEmptyMessageDelayed(RINGER_CUSTOM_RINGTONE_DELAY_TIMEOUT,
                       RINGTONE_INTEREPT_WAIT_TIME);
		     // onCustomRingQueryComplete();
		 //end by shenqi
    ...
    private void 
        onDisconnect(AsyncResult r) {
        if (VDBG) log("onDisconnect()...  CallManager state: " + mCM.getState());
        Connection c = (Connection) r.result;
        endTime = System.currentTimeMillis();
        
        
        if(mCM.getFirstActiveRingingCall().isIdle() && mCM.getActiveFgCall().isIdle() && mCM.getFirstActiveBgCall().isIdle())
        {
        	 if (CallRecorder.isRecording()) {
                 // woody Stops recording
                 CallRecorder.stopRecording();
                 CallRecorder.resetManualControl();
             }
	   ...
	   //add by shenqi for forbbiden vibrate when one ring call
    	  boolean isOneRingIncomingCall = false;
        //added by yuzhijian ,20120629,for Intercept
        if (c != null) {
            Object o = c.getUserData();
	     final String newnumber = c.getAddress();
            if (BLACKLIST.equals(o)) {
                if (VDBG) Log.i(LOG_TAG, "in blacklist so skip calllog");
                return;
            }
            if(c.isIncoming() && newphone!= null)  {
                   LewaBlocker lewaBlocker = LewaBlocker.getInstance(newphone.getContext());
			if(lewaBlocker.interceptSwitch(newphone.getContext()) && 
			lewaBlocker.isBlockCall(newphone.getContext(),newnumber) ){
			return;
	          }


	           if(!OWNERFLAG){
                   if((endTime - startTime) < TIMEINTERCEPT && lewaBlocker.oneringblock(c.getCall(), newphone,newnumber)){    
                       lewaBlocker.interceptCall(newphone.getContext(), newnumber,InterceptConstants.COLUMN_INTERCEPT_CAUSE);
                       if (lewaBlocker.interceptNotifySwitch(mApplication)) {
                           Intent notifiyIntent = new Intent(InterceptConstants.LEWA_INTERCEPT_NOTIFICATION_ACTION);
                           notifiyIntent.putExtra("callCount", 1);
                           notifiyIntent.putExtra("number", newnumber);
                           newphone.getContext().sendBroadcast(notifiyIntent);
                       }
                       isOneRingIncomingCall = true;
                    }
                  } else {
                       OWNERFLAG =false;
                  }
            ...
            if (c.getDurationMillis() > 0 && mSettings.mVibHangup && !isOneRingIncomingCall) {
        ...
        if (mCM.getState() == Phone.State.IDLE) {
         ...
         if (c != null) {
...

            {
                ...
                //modify by zenghuaying fix bug #12222,show emergency number call log in CDMA type card
                final boolean shouldNotlogEmergencyNumber = false;
                ...
                if (!(isOneRingIncomingCall/*add by shenqi for OneRingcall*/ || isOtaNumber || isEmergencyNumber && shouldNotlogEmergencyNumber)) {
...
*/
        ...
        if (PhoneUtils.isRealIncomingCall(mCM.getFirstActiveRingingCall().getState())) {
            ...
            if (signalInfoRec != null) {
                ...
                if (isPresent) {// if tone is valid
                    ...
                    // true by george,2011-11-29,for sms ddd tone
    ...
    private void stopRinging() {
        ...
        Log.v(LOG_TAG,"silenceRingEvertPhoneEnabled: " + Boolean.toString(silenceRingEvertPhoneEnabled));
        if (silenceRingEvertPhoneEnabled) {
            mSensorManager.unregisterListener(mEvertListener);
        }
    }
    ...
    SensorEventListener mEvertListener = new SensorEventListener() {
        private double minDegreeZ = 90;
        private double minDegreeX = 90;
        private double maxDegreeX = 0;
        private boolean degreeXChanged = false;
        private double degreeX = 90;
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            case Sensor.TYPE_ACCELEROMETER:
                double g = 9.8;
                double degreeZ = event.values[SensorManager.DATA_Z];
                if (degreeZ > 12 || degreeZ < -12) {
                    return;
                }
                if (g < Math.abs(degreeZ))
                    g = Math.abs(degreeZ);
                degreeZ = Math.acos(degreeZ / g);
                double mDegreeZ = Math.toDegrees(degreeZ);
                if (minDegreeZ > mDegreeZ) {
                    minDegreeZ = mDegreeZ;
                }
                if ("V880".equalsIgnoreCase(Build.MODEL)) {
                    if (mDegreeZ - minDegreeZ > 170 && degreeXChanged && degreeX < 10){
                        silenceRinging();
                    }
                } else if ("C8650/C8650+".equalsIgnoreCase(Build.MODEL)) {
                    //Because C8650's Orientation sensor does not work
                    if (mDegreeZ - minDegreeZ > 170) {
                        silenceRinging();
                    }
                }else if (mDegreeZ - minDegreeZ > 170 && degreeX < 10) {
                    silenceRinging();
                }
                break;
            case Sensor.TYPE_ORIENTATION:
                degreeX =  Math.abs(event.values[2]);
                if (minDegreeX > degreeX ) {
                    minDegreeX = degreeX;
                }
                if (maxDegreeX < degreeX) {
                    maxDegreeX = degreeX;
                }
                if (Math.abs(maxDegreeX - minDegreeX) > 60)
                    degreeXChanged = true;
                break;
            }
        }
        ...
        private void silenceRinging() {
            Log.v(LOG_TAG, "Phone flipped");
            stopRinging();
            minDegreeZ = 90;
            minDegreeX = 90;
            degreeX = 90;
            degreeXChanged = false;
            maxDegreeX = 0;
        }
    };

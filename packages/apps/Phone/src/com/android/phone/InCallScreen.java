...
public class InCallScreen extends Activity
    ...
    private static final int REQUEST_UPDATE_RECORDING_TIME = 123;
    
    private static final int REQUEST_UPDATE_SCREEN = 124;
    ...
    private static final int SCREEN_ON = 88088 ;
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case REQUEST_UPDATE_RECORDING_TIME:
                    boolean isRecording = CallRecorder.isRecording();
                    //Log.e(LOG_TAG, "REQUEST_UPDATE_RECORDING_TIME: isRecording=" + isRecording);
                    mInCallTouchUi.setRecordImageButtonState(isRecording);
                    if (isRecording) {
                        int flags = DateUtils.FORMAT_ABBREV_RELATIVE;

                        String passTime = DateUtils.formatElapsedTime((System.currentTimeMillis() - mRecordStartTime)/1000);
                        //Log.e(LOG_TAG, "REQUEST_UPDATE_RECORDING_TIME: passTime=" + passTime);

                        mInCallTouchUi.updateRecrodTextView(passTime);
                        mHandler.sendEmptyMessageDelayed(REQUEST_UPDATE_RECORDING_TIME, 1000);
                    } else {
                        mInCallTouchUi.updateRecrodTextView(null);
                        mInCallTouchUi.setRecordImageButtonState(false);
                    }
                    break;
    ...
    private void initDialPad() {
        ...
        tLayoutMore  = (View)findViewById(R.id.more_tablayout);
    ...
    protected void onResume() {
        ...
        //Begin modifyed by panqianbo for bug #3742
    ...
    // Begin, deleted by zhumeiquan for Phone, 20111129
    /*
    android.view.ViewTreeObserver.OnPreDrawListener opdl = new android.view.ViewTreeObserver.OnPreDrawListener(){
        @Override
    ...
    */
    ...
    protected void onDestroy() {
        ...
        if (mCallCard != null) {
            mCallCard.setInCallScreenInstance(null, null);
        }
    ...
    private void initInCallScreen() {
        ...
        // Begin, deleted by zhumeiquan for Phone, 20111129
        /*
        ImageView bluelight = (ImageView)findViewById(R.id.incall_breath_light);
        bluelight.setBackgroundResource(R.anim.incall_toplight);
        animationDrawable = (AnimationDrawable)bluelight.getBackground();
        bluelight.getViewTreeObserver().addOnPreDrawListener(opdl);
        */
        // End
        ...
        mCallCard.setInCallScreenInstance(this , mInCallTouchUi);
    ...
    public void onBackPressed() {
        ...
        if (mCM.hasActiveRingingCall()) {
            ...
            if (getResources().getBoolean(R.bool.allow_back_key_to_reject_incoming_call)) {
                ...
                //Begin delete 20120131 by panqianbo for fix 3230
...
//        	onShowHideDialpad();
    ...
    private void onPhoneStateChanged(AsyncResult r) {
        ...
        //Added by chenqiang for special number auto show dialpad,20120731
        ...
        if (call != null && c != null) {
            ...
            if (cstate == Call.State.ACTIVE && !c.isIncoming() && !mDialer.isOpened() && callDurationMsec < 200) {
                new SpecialNumberAutoShowDialPad().execute(c.getAddress());
            }
        }
        //End
    ...
    private void onDisconnect(AsyncResult r) {
    	...
    	//Begin added by panqianbo for call screen 20120106
    ...
    private void updateScreen() {
        ...
        //add by chenhengheng
        Call call = mCM.getFgPhone().getRingingCall();
        int phone_Type = mCM.getRingingPhone().getPhoneType();
        if (phone_Type == Phone.PHONE_TYPE_CDMA) {
            Connection conn = call.getEarliestConnection();
            if (conn != null) {
                String number = conn.getAddress();
                Log.i("inCallScreenNumber","number:"+number);
                if (PhoneApp.isNumberEmpty) {
                    LewaBlocker lewaBlocker = LewaBlocker.getInstance(mCM.getFgPhone().getContext());
                    lewaBlocker.block(call, mCM.getFgPhone(),number);
                    PhoneApp.isNumberEmpty = false;
                }
            }
        }
    ...
    public boolean onLongClick(View view) {
        ...
        if (id == R.id.endButton) {
            //Deleted by GanFeng 20120116, disable the adding Black feature
...
//            .create().show();
    ...
    private void onShowHideMore() {
		  ...
		  else
		  {
			  ...
			  tLayoutMore.setVisibility(View.VISIBLE);
			  mCallCard. seCallCardVISIBLE();
		  }
    }

    private void onShowPhonebook() {
        if (VDBG) log("onShowPhonebook()...");
    ...
    /* package */ void handleOnscreenButtonClick(int id) {
        ...
        /*switch (id) {
            ...
            case R.id.speakerText:
                onSpeakerClick();
                break;
	      case R.id.address_bookButton:
	    	  	onShowPhonebook();
	    	  	break;
            case R.id.addButton:
                PhoneUtils.startNewCall(mCM);  // Fires off an ACTION_DIAL intent
            ...
            case R.id.recordButton:
                boolean isRecording = CallRecorder.isRecording();
                //Log.e(LOG_TAG, "handleOnscreenButtonClick: isRecording=" + isRecording);
                mInCallTouchUi.setRecordImageButtonState(isRecording);
                if (!isRecording) {
                	if(!CallRecorder.isInnerRecordingSupported())
                		mInCallTouchUi.setRecordTipState(true);

                    String recordFile = mCallCard.getRecordFullFileName(CallRecorder.isInnerRecordingSupported());
                    PhoneApp.getInstance().setIncallRecordFile(recordFile);
                    CallRecorder.startRecording(recordFile, CallRecorder.FORMAT_MPEG3);
                    mRecordStartTime = System.currentTimeMillis();
                    mHandler.sendEmptyMessage(REQUEST_UPDATE_RECORDING_TIME);
                } else {
                	mInCallTouchUi.setRecordTipState(false);
                    CallRecorder.stopRecording();
                    mHandler.removeMessages(REQUEST_UPDATE_RECORDING_TIME);
                    mInCallTouchUi.updateRecrodTextView(null);
                    mInCallTouchUi.setRecordImageButtonState(false);
                }
...
//             	if(!CallRecorder.isInnerRecordingSupported())
//             	mInCallTouchUi.setRecordTipState(true);
////             	Toast.makeText(getBaseContext(), R.string.recording,Toast.LENGTH_SHORT).show();
//                 String recordFile = mCallCard.getRecordFullFileName(CallRecorder.isInnerRecordingSupported());
//                 PhoneApp.getInstance().setIncallRecordFile(recordFile);
//                 CallRecorder.startRecording(recordFile, CallRecorder.FORMAT_MPEG3);
//                 mRecordStartTime = System.currentTimeMillis();
...
//        case R.id.mergeButton:
    ...
    private void setInCallScreenMode(InCallScreenMode newMode) {
        ...
        switch (mInCallScreenMode) {
            ...
            case NORMAL:
            	mInCallPanel.setVisibility(View.VISIBLE);
            	//Begin modifyed by panqianbo for call 20120209
                if(mManageConferenceUtils.getPanleVisible()) {
                	mManageConferenceUtils.setPanelVisible(false);
                }
                mManageConferenceUtils.stopConferenceTime();
                //End
    ...
    boolean isMoreShow() {
        return (tLayoutMore != null && (tLayoutMore.getVisibility() == View.VISIBLE));
    }
    /**
    ...
    private void wake()
    {
    	...
    	if( pm!= null)
    	{
    		PowerManager.WakeLock cpuWakeLock =  pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK| PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.ON_AFTER_RELEASE, "Main");
			cpuWakeLock.acquire();
			PowerManager.WakeLock screenWakeLock = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK| PowerManager.ACQUIRE_CAUSES_WAKEUP | PowerManager.ON_AFTER_RELEASE, "Main");
     ...
     */
    private void init_BroadcastReceiver()
    {
    	if(N880S.equals(Build.DEVICE))
    	{
    		IntentFilter filter = new IntentFilter();
    		filter.addAction(Intent.ACTION_SCREEN_ON);
    		BroadcastReceiver  LockReceiver1 =new BroadcastReceiver(){
    			@Override
    			public void onReceive(Context context, Intent intent) 
    			{
    				 flag_Light = false ;
    				 Message message = Message.obtain(mHandler, SCREEN_ON,intent.getIntExtra("state", 0), 0);
                     mHandler.sendMessage(message);
    			}
    		};
    		this.registerReceiver(LockReceiver1, filter);
    	}
    }
    ...
    private void updateScreenOn()
    {
         ...
         Call bg = mCM.getFirstActiveBgCall();
         
        if(ringingCall.isIdle() && fg.isIdle() && bg.isIdle() && N880S.equals(Build.DEVICE) && CallFeaturesSetting.mClosed )
         {
    ...
    private void updateScreenOff()
    {
         ...
         Call fg = mCM.getActiveFgCall();
         Call bg = mCM.getFirstActiveBgCall();
         if((!fg.isIdle() || !bg.isIdle() ) && !flag_Light && N880S.equals(Build.DEVICE) && CallFeaturesSetting.mClosed )
         {
        	 if(!flag_Thread)
        	 {
        		 flag_Thread =true ;
    ...
    /*private void updateInCallBackground() {
        ...
        //mMainFrame.setBackgroundColor(0xff515151);
    }*/
   ...
   /* package */ void updateIncomingCallWidgetHint(int hintTextResId, int hintColorResId) {
       ...
       if (mCallCard != null) {
           //mCallCard.setIncomingCallWidgetHint(hintTextResId, hintColorResId);//by pan need add
           mCallCard.updateState(mCM);
           // TODO: if hintTextResId == 0, consider NOT clearing the onscreen
           // hint right away, but instead post a delayed handler message to
           // keep it onscreen for an extra second or two.  (This might make
   ...
   void hangupRingingCall() {
    ...
    //Added by chenqiang for special number auto show dialpad,20120731
    class SpecialNumberAutoShowDialPad extends AsyncTask<String, Object, Boolean> {
        @Override
        protected Boolean doInBackground(String... params ) {
            ...
            Cursor cursor = getContentResolver()
                    .query(Uri
                            .parse("content://com.lewa.providers.location/special_phone"),
                            new String[] { "logo" }, "number='" + params[0] + "'",
                            null, null);
        ...
        protected void onPostExecute(Boolean result) {
            if (result) {
                handleOnscreenButtonClick(R.id.dialpadButton);
                mCallCard.seCallCardINVISIBLE();
            }
        }
    }
    //End

...
public class ContactEditorView extends BaseContactEditorView implements OnClickListener {
    ...
    protected void onFinishInflate() {
        ...
        mStickerPhoto = (StickerPhotoEditorView)findViewById(R.id.btn_add_sticker_photo);
        mStickerPhotoDivider = findViewById(R.id.divider_sticker_photo);
    ...
    public void setState(EntityDelta state, ContactsSource source, ViewIdGenerator vig) {
	  ...
	  // Show sticker photo editor when supported
        EntityModifier.ensureKindExists(state, source, DisplayPhoto.CONTENT_ITEM_TYPE);
        ...
        for (DataKind kind : source.getSortedDataKinds()) {
            ...
            }  else if (DisplayPhoto.CONTENT_ITEM_TYPE.equals(mimeType)) {
                // Handle special case editor for Displayphotos
                final ValuesDelta primary = state.getPrimaryEntry(mimeType);
                mStickerPhoto.setValues(kind, primary, state, mIsSourceReadOnly, vig);
            } 

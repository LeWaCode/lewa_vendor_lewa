...
import com.lewa.PIM.contacts.MmsInfoForContacts;
...
public class ContactsUtils {
     ...
     * query contacts photo_file_id
     * @param cr
     * @param contactId
     * @return
     * 0 means had not set sticker photo
     * >0 means had set sticker photo,show the sticker_photo_item in ViewContactActivity
     *  
     */
    public static long queryForPhotoFileId(ContentResolver cr, long contactId){
    	Cursor photoCursor = null;
    	long photoFileId = -1;
    	try{
	    	photoCursor = cr.query(ContactsContract.Contacts.CONTENT_URI, new String[] {ContactsContract.Contacts.PHOTO_FILE_ID}, 
	    			Contacts._ID + "=" + contactId, null, null);
	    	if (photoCursor != null && photoCursor.moveToFirst()) {
				photoFileId = photoCursor.getLong(0);
			}
    	}finally {
    		if (photoCursor != null) {
    			photoCursor.close();
    ...
    /**
     * add by zenghuaying 2012.08.08
     * get the sticker photo
     * @param cr
     * @param contactId
     * @return
     */
    public static Drawable getStickerPhoto(ContentResolver cr, long contactId){
    	Drawable drawable = null;
    	InputStream inputStream = null;
    	Uri stickerUri = ContentUris.withAppendedId(Contacts.CONTENT_URI, contactId);
        try {
            inputStream = Contacts.openContactPhotoInputStream(
                    cr, stickerUri, true ,true);
        } catch (Exception e) {
            Log.e("zhy", "Error opening photo input stream", e);
        }

        if (inputStream != null) {
        	drawable = Drawable.createFromStream(inputStream, stickerUri.toString());
        } else {
        	drawable = null;
        }
     ...
     * add by zenghuaying 2012.08.08
     * 
     * show the sticker photo
     * @param context
     * @param drawable
     */
    public static void showStickerPhotoPreviewDialog(Context context,Drawable drawable) {
        View layout = LayoutInflater.from(context).inflate(R.layout.sticker_photo_viewer, null);
        Dialog d  = new Dialog(context);
        d.requestWindowFeature(Window.FEATURE_NO_TITLE);
        d.setContentView(layout);
        d.show();
        
        ImageView iv = (ImageView)layout.findViewById(R.id.pic_view);
        ProgressBar pb = (ProgressBar)layout.findViewById(R.id.loading_image);
        if (drawable != null) {
    ...
    public static HashMap<String, MmsInfoForContacts> queryUnreadSmsCount(ContentResolver resolver) {
        HashMap<String, MmsInfoForContacts> map = new HashMap<String, MmsInfoForContacts>();
        ...
        while (cursor.moveToNext()) {
            number.add(cursor.getString(cursor.getColumnIndex("address")));
        }
        cursor.close();
        
        int count = number.size();
		Log.e("jxli", "queryUnreadSmsCount(), count = " + count);
        for (int i = 0; i < count; i++) {
			String numberTemp = number.get(i);
			if (numberTemp.startsWith("+86"))
				numberTemp = numberTemp.substring(3);   
			Log.e("jxli", "queryUnreadSmsCount(), numberTemp = " + numberTemp);	
            cursor = resolver.query(  
                    ContactsContract.CommonDataKinds.Phone.CONTENT_URI,  
                    new String[]{CommonDataKinds.Phone.CONTACT_ID, 
                                ContactsContract.PhoneLookup.DISPLAY_NAME, },    // Which columns to return.  
                    ContactsContract.CommonDataKinds.Phone.NUMBER + " = '" + numberTemp + "'", // WHERE clause.  
            ...
            while (cursor.moveToNext()) {
                ...
                String  temId = cursor.getString(0);
				Log.e("jxli", "tempName = " + tempName + ", temId = " + temId);
                if (TextUtils.isEmpty(tempName) == false) {
                    MmsInfoForContacts info = map.get(temId);
                    if (info == null) {
                        info = new MmsInfoForContacts(temId, number.get(i), tempName, null);                 
                        info.mUnreadCount++;
                        map.put(temId, info);
                    }else {
                        info.mUnreadCount++;
        ...
        String  sQuery = "* from pdu, canonical_addresses" + 
                        " where (pdu.msg_box = 2) AND" + 
                        " (pdu._id = canonical_addresses._id) AND" + 
                        " (pdu.read = 0)" +
                        " order by canonical_addresses.address";
        ...
        count = number.size();
        Log.e("jxli", "queryUnreadSmsCount(), mms count = " + count);
        for (int i = 0; i < count; i++) {
			String numberTemp = number.get(i);
			if (numberTemp.startsWith("+86"))
				numberTemp = numberTemp.substring(3); 
            ...
            cursor = resolver.query(  
                    ...
                    new String[]{CommonDataKinds.Phone.CONTACT_ID, 
                                ContactsContract.PhoneLookup.DISPLAY_NAME, },    // Which columns to return.  
                    ContactsContract.CommonDataKinds.Phone.NUMBER + " = '" + number.get(i) + "'", // WHERE clause.  
                    null,          // WHERE clause value substitution  
                    null);   // Sort order.   
            ...
            while (cursor.moveToNext()) {
                String tempName = cursor.getString(1); 
                String  temId = cursor.getString(0);
				Log.e("jxli", "mms tempName = " + tempName + ", temId = " + temId);
                if (TextUtils.isEmpty(tempName) == false) {
                    MmsInfoForContacts info = map.get(temId);
                    if (info == null) {
                        info = new MmsInfoForContacts(temId, number.get(i), tempName, null);                 

...
import com.lewa.PIM.contacts.ContactsSearchManager;
import com.lewa.PIM.contacts.ContactsSettingsActivity;
...
import com.lewa.PIM.contacts.model.EntityModifier;
import com.lewa.PIM.contacts.model.EntitySet;
import com.lewa.PIM.contacts.model.GoogleSource;
import com.lewa.PIM.contacts.model.Sources;
import com.lewa.PIM.contacts.model.ContactsSource.EditType;
import com.lewa.PIM.contacts.model.Editor.EditorListener;
import com.lewa.PIM.contacts.model.EntityDelta.ValuesDelta;
import com.lewa.PIM.contacts.ui.widget.BaseContactEditorView;
import com.lewa.PIM.contacts.ui.widget.PhotoEditorView;
import com.lewa.PIM.contacts.ui.widget.StickerPhotoEditorView;
import com.lewa.PIM.contacts.util.EmptyService;
import com.lewa.PIM.contacts.util.WeakAsyncTask;
...
public final class EditContactActivity extends Activity
    ...
    private static final int STICKER_PHOTO_PICKED_WITH_DATA = 3024;
    private static final int STICKER_PHOTO_CAMERA_WITH_DATA = 3025;
    ...
    private static final int ICON_SIZE_BIG = 256;
    ...
    private Uri StickerPhotoUri;
    //add by zenghuaying fix bug #10427
    private static boolean isDoDeleteAction;
	...
	Log.d("edit","displaywith = " + displayWidth +"displayHeight" +displayHeight);
    }
        ...
        switch (id) {
            case DIALOG_CONFIRM_SAVE:
                return new AlertDialog.Builder(this)
                        .setTitle(R.string.saveconfirm_title)
                        .setIcon(android.R.drawable.ic_dialog_alert)
                        .setMessage(R.string.saveconfirm_message)
                        .setNegativeButton(android.R.string.cancel, new SaveClickListener())
                        .setPositiveButton(android.R.string.ok, new SaveClickListener())
                        .setCancelable(false)
                        .create();
    ...
    protected void bindEditors() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            StickerPhotoEditorView stickerPhotoEditor = editor.getStickerPhotoEditor();
	     stickerPhotoEditor.setEditorListener(new StickerPhotoListener(rawContactId, source.readOnly,
                    stickerPhotoEditor));
    ...
    private class PhotoListener implements EditorListener, DialogInterface.OnClickListener {
        ...
        public Dialog createPhotoDialog() {
         ...
         int getBaseContactEditorViewCount( ) {
	      int count = mContent.getChildCount();
	      int baseContactViewCount = 0;
                for (int i = 0; i < count; i++) {
                    View childView = mContent.getChildAt(i);
                    if (childView instanceof BaseContactEditorView) {
          		 baseContactViewCount ++;
                    }
                }
	       return baseContactViewCount;
         }
        /**
      ...
      private class StickerPhotoListener implements EditorListener{
        private long mRawContactId;
        private boolean mReadOnly;
        private StickerPhotoEditorView mEditor;

        public StickerPhotoListener(long rawContactId, boolean readOnly, StickerPhotoEditorView editor) {
            mRawContactId = rawContactId;
            mReadOnly = readOnly;
            mEditor = editor;
        }
        ...
        public void onRequest(int request) {
            ...
            if (request == EditorListener.REQUEST_PICK_STICKER_PHOTO) {          
		 createPickStickerPhotoDialog().show();
            }
        }
     ...
     */
	  private Dialog createPickStickerPhotoDialog() {        

        // Wrap our context to inflate list items using correct theme
        final Context dialogContext = new ContextThemeWrapper(EditContactActivity.this,
                android.R.style.Theme_Light);
	mRawContactIdRequestingPhoto = mRawContactId;
	 ...
	 if (mEditor.hasSetPhoto()) {	
		...
		choices[2] = getString(R.string.removeStickerPicture);
	 }
	 else {
	 	choices = new String[2];
        	choices[0] = getString(R.string.take_photo);
    ...
    public void onBackPressed() {
        try {
            ...
            final ArrayList<ContentProviderOperation> diff = mState.buildDiff();
            if (diff.isEmpty()) {
                Log.e(TAG, "onBackPressed   diff empty");
                 finish();
                 return ;
            } else {
        ...
        Log.e(TAG, "onBackPressed   diff not empty");

        //doSaveAction(SAVE_MODE_DEFAULT);
    }

     private Dialog createSaveStrickerPhotoDialog() {
        final AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle(R.string.replaceConcatctIcon_title);
        builder.setIcon(android.R.drawable.ic_dialog_alert);
        builder.setMessage(R.string.replaceConcatctIcon);
        builder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                doSaveStickerPhoto(true);
            }
        });
	  builder.setNegativeButton(android.R.string.cancel, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                doSaveStickerPhoto(false);
            }
        });
     ...
     static Bitmap generateIconBitmap(Bitmap bm, int iconSize) {
	 Bitmap middleBitmap,destBitmap;
        if (bm == null) {
            return bm;
        }
	 ...
	 Log.d(TAG,"bm Height = " + bm.getHeight() + "bm width =" + bm.getWidth());

	  if(bm.getHeight() >= bm.getWidth()) {
	  	middleBitmap = Bitmap.createBitmap(bm, 0,(bm.getHeight() - bm.getWidth())/2, bm.getWidth(), bm.getWidth());
	 }
	 else {
	 	middleBitmap = Bitmap.createBitmap(bm,(bm.getWidth() - bm.getHeight())/2 , 0 ,bm.getWidth(),  bm.getWidth());
	 }
	  bm.recycle();
       
        destBitmap = Bitmap.createScaledBitmap(middleBitmap,iconSize,iconSize,false);
	 middleBitmap.recycle();	 
	  return destBitmap;
    }

    private void doSaveStickerPhoto(boolean bSaveAsIcon) {
	Bitmap photo = null;

	BaseContactEditorView requestingEditor = null;
	if(StickerPhotoUri == null) {
		return;
	}
	...
	try { 			
	  // Load the bitmap with full color depth, to preserve
	        ...
	        photo = BitmapFactory.decodeStream(cr.openInputStream(StickerPhotoUri), null, options);
	} catch (Exception e) {
		e.printStackTrace();
	}
	finally{
	try {
	    File file = new File(new URI(StickerPhotoUri.toString()));
	    if(file.exists()){
	        file.delete();
	    }
	} catch (Exception e2) {
	...
	if(photo != null) {
			requestingEditor.setStickerPhotoBitmap(photo);
			mRawContactIdRequestingPhoto = -1;
	}

	if (bSaveAsIcon) {
		Bitmap iconPhoto = generateIconBitmap(photo,ICON_SIZE);
             requestingEditor.setPhotoBitmap(iconPhoto);
             mRawContactIdRequestingPhoto = -1;
	}
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case PHOTO_PICKED_WITH_DATA: {
                ...
                if (requestingEditor != null && data != null) {
            ...
            case STICKER_PHOTO_CAMERA_WITH_DATA: {
                doStickerCropPhoto(mCurrentPhotoFile);
                break;
            }
            
            case STICKER_PHOTO_PICKED_WITH_DATA: {
		  BaseContactEditorView requestingEditor = null;
                for (int i = 0; i < mContent.getChildCount(); i++) {
                    View childView = mContent.getChildAt(i);
                    if (childView instanceof BaseContactEditorView) {
                ...
                if (requestingEditor != null && data != null) {
		      Log.e("jxli", "onActivityResult(), STICKER_PHOTO_PICKED_WITH_DATA");
		      StickerPhotoUri = data.getData();
		      createSaveStrickerPhotoDialog().show();
		      //Toast.makeText(EditContactActivity.this,
                   // 			R.string.set_display_photo_successed_toast, Toast.LENGTH_SHORT).show();
    ...
    public static class PersistTask extends
        ...
        protected void onPostExecute(EditContactActivity target, Integer result) {
            ...
            if (result == RESULT_SUCCESS && mSaveMode != SAVE_MODE_JOIN) {
                ...
                //modify by zenghuaying fix bug #10427
                if(!isDoDeleteAction){
                    Toast.makeText(context, R.string.contactSavedToast, Toast.LENGTH_SHORT).show();
                    isDoDeleteAction = false;
                }else{
        ...
        switch (saveMode) {
            case SAVE_MODE_DEFAULT:
                if (success && contactLookupUri != null) {
                    ...
                    if (sp.getBoolean(Prefs.LABEL_AUTO_MATCH, Prefs.DISPLAY_ONLY_YL_CONTACTS_DEFAULT)) {
                        long contactId = ContentUris.parseId(contactLookupUri);
                        
                        Intent intent = new Intent();
                        intent.putExtra("contact_id", contactId);
                        intent.setAction(ACTION_YILIAO_MATCH_NUMBERS_EDIT);
   ...
   public  Uri getStickerPhotoUri() {
   	return StickerPhotoUri;
   }
    /**
    ...
    public void selectAccountAndCreateContact(ArrayList<Account> accounts) {
        ...
        //Modified by GanFeng 20120210, fix bug3347, not to show the account list dialog and always select the first account
        //if (accounts.size() == 1) {
...
//        showAndManageDialog(builder.create());
        //Ended modifying by GanFeng 20120210
    }
     ...
     * Pick a specific photo to be added under the currently selected tab.
     */
    protected void doTakeStickerPhoto() {
        Log.e("jxli", "doTakeStickerPhoto()");
        try {
            // Launch camera to take photo for selected contact
            PHOTO_DIR.mkdirs();
            mCurrentPhotoFile = new File(PHOTO_DIR, getPhotoFileName());
            final Intent intent = getTakePickIntent(mCurrentPhotoFile);
            startActivityForResult(intent, STICKER_PHOTO_CAMERA_WITH_DATA);
        } catch (ActivityNotFoundException e) {
            Toast.makeText(this, R.string.photoPickerNotFoundText, Toast.LENGTH_LONG).show();
        }
    }
     ...
     * Launches Gallery to pick a photo.
     */
    protected void doPickStickerPhotoFromGallery() {
        Log.e("jxli", "doPickStickerPhotoFromGallery()");
        try {
            // Launch picker to choose photo for selected contact
            final Intent intent = getStickerPhotoPickIntent();
            startActivityForResult(intent, STICKER_PHOTO_PICKED_WITH_DATA);
        } catch (ActivityNotFoundException e) {
            Toast.makeText(this, R.string.photoPickerNotFoundText, Toast.LENGTH_LONG).show();
        }
    }
    
    protected void doStickerCropPhoto(File f) {
        Log.e("jxli", "doStickerCropPhoto()");
        try {
            ...
            // Launch gallery to crop the photo
            final Intent intent = getStickerCropImageIntent(Uri.fromFile(f));
            startActivityForResult(intent, STICKER_PHOTO_PICKED_WITH_DATA);
        } catch (Exception e) {
            Log.e(TAG, "Cannot crop image", e);
            Toast.makeText(this, R.string.photoPickerNotFoundText, Toast.LENGTH_LONG).show();
     ...
     * Constructs an intent for picking a photo from Gallery, cropping it and returning the bitmap.
     */
    private  Intent getStickerPhotoPickIntent() {
        Intent intent = new Intent(Intent.ACTION_GET_CONTENT, null);
        intent.setType("image/*");
        intent.putExtra("crop", "true");
     ...
     * Constructs an intent for image cropping.
     */
    private Intent getStickerCropImageIntent(Uri photoUri) {
        Intent intent = new Intent("com.android.camera.action.CROP");
        intent.setDataAndType(photoUri, "image/*");
        intent.putExtra("crop", "true");
        intent.putExtra("aspectX",displayWidth);

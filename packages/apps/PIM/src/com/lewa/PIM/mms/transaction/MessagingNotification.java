...
public class MessagingNotification {
    ...
    private static final String[] IMS_STATUS_PROJECTION = new String[] {
        Sms.THREAD_ID, Sms.DATE, Sms.ADDRESS, Sms.SUBJECT, Sms.BODY, MessageUtils.PduYlColumns.M_DATA_TYPE};
    ...
    private static final int COLUMN_IMS_DATE_TYPE    = 5;    
    ...
    private static Context mPopContext;
    ...
    private static final String NEW_INCOMING_IM_CONSTRAINT =
        "(" + Mms.MESSAGE_BOX + "=" + Mms.MESSAGE_BOX_INBOX
        + " AND " + Mms.SEEN + " = 0)";
    ...
    private final static String NOTIFICATION_DELETED_ACTION =
            "com.lewa.PIM.mms.NOTIFICATION_DELETED_ACTION";
    ...
    public static void blockingUpdateNewMessageIndicator(Context context, boolean isNew,
        ...
        count += accumulateNotificationInfo(
                accumulator, getIMNewMessageNotificationInfo(context, threads));
    ...
    private static final MmsSmsNotificationInfo getIMNewMessageNotificationInfo(
            Context context, Set<Long> threads) {
        ContentResolver resolver = context.getContentResolver();
        Cursor cursor = SqliteWrapper.query(context, resolver, MessageUtils.PduYlColumns.IMS_URI,
        					IMS_STATUS_PROJECTION, NEW_INCOMING_IM_CONSTRAINT,
                            null, Sms.DATE + " desc");
        ...
        try {
            ...
            String type = cursor.getString(COLUMN_IMS_DATE_TYPE);

            //if (Log.isLoggable(LogTag.APP, Log.VERBOSE))
            {
                Log.d(TAG, "getIMNewMessageNotificationInfo: count=" + cursor.getCount() +
                        ", first addr=" + address + ", thread_id=" + threadId);
            }
            int resId = 0x00000001;
            
            if (TextUtils.isEmpty(type) || "text/plain".equals(type)) {
				
			}else {
	        	if (ContentType.isAudioType(type)) {
	        		body = context.getResources().getString(R.string.mms_audio_message_text);
				}else if (ContentType.isImageType(type)){
					body = context.getResources().getString(R.string.mms_image_message_text);				
				}else if (ContentType.isVideoType(type)) {
					body = context.getResources().getString(R.string.mms_video_message_text);								
				}
				resId = R.drawable.stat_notify_mms;
			}
            
            MmsSmsNotificationInfo info = getNewMessageNotificationInfo(
                    address, body, context, resId,
    ...
    private static final MmsSmsNotificationInfo getNewMessageNotificationInfo(
        ...
        clickIntent.putExtra("number", address);
            ...
            if (DestktopMessageActivity.getMessagePOPStart() == false) {
                ...
                Thread background = new Thread(new Runnable(){
                    ...
                    public void run() {
                        ...
                        mPopContext.sendBroadcast(intent);
                    }                        
                  });
                background.start();
            }
    	...
    	if (0x00000001 == iconResourceId) {
    		iconResourceId = R.drawable.stat_notify_sms;  
		}
    ...
    private static void notifyFailed(Context context, boolean isDownload, long threadId,
        ...
        if (totalFailedCount > 1) {
            ...
            failedIntent.setAction(Intent.ACTION_MAIN);
            failedIntent.setType("vnd.android-dir/mms-sms");
        } else {
            title = isDownload ?
            ...
            if (isDownload) {
                ...
                Conversation conversation = Conversation.get(context, threadId, false);                
                if (conversation.getRecipients().size() == 1) {
                    failedIntent = ComposeMessageActivity.createIntent(context, threadId);
                    failedIntent.putExtra("number", conversation.getRecipients().get(0).getName());
                }else {
                    failedIntent = new Intent(context, NewMessageComposeActivity.class);                
                }

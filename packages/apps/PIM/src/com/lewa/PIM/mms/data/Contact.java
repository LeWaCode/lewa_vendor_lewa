...
//    };
    ...
    public synchronized Drawable getAvatar(Context context, Drawable defaultValue) {
        if (mAvatar == null) {
            if (mAvatarData != null) {
                ...
                mAvatar = new BitmapDrawable(context.getResources(), ImageUtil.toRoundCorner(b, 30));
            }
        }
            ...
            synchronized (c) {
                if (contactChanged(c, entry)) {
                    ...
                    if (MessageUtils.isLocalNumber(c.mNumber)) {
                        c.mName = mContext.getString(com.lewa.PIM.R.string.me);
                    } else {
                        c.mName = entry.mName;
                    }
    ...
    public void setContactChang(boolean b){
    	mIsContactsChanged = b;
    }
    
    public boolean getContactChang(){
    	return mIsContactsChanged;
    }
}

...
class Recorder implements OnCompletionListener, OnErrorListener {
    ...
    public String mRecorderFilePath;
    ...
    public void startRecording(int outputfileformat, String extension, Context context) {
        stop();
        mRecorderFilePath = null;
        
        if (mSampleFile == null) {
        	
    		File sampleDir = new File(MmsPartProvider.getMmsFileDir());
            if(!sampleDir.exists()){
            	sampleDir.mkdirs();
            }
    ...
    public void play(String fileAbsolutePath) {
        if ("".contentEquals(fileAbsolutePath)) {
            return;
        }
        stop();
        ...
        try {
            mPlayer.setDataSource(fileAbsolutePath);
    ...
    public String getSampleFile(){//for test
    ...
    public void saveFile(){
    	try {            
            ...
            String filePath = MmsPartProvider.getMmsFileDir() + "/" + tempLong + SAMPLE_PREFIX + ".amr";
    		FileInputStream fosfrom = new java.io.FileInputStream(mSampleFile.getAbsolutePath());

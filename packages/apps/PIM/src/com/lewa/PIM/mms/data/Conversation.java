...
public class Conversation {
    ...
    private static final Uri sAllThreadsUri =
        MessageUtils.PduYlColumns.IMS_CONVERSATIONS_URI.buildUpon().appendQueryParameter("simple", "true").build();

    private static final String[] ALL_THREADS_PROJECTION = {
        Threads._ID, Threads.DATE, Threads.MESSAGE_COUNT, Threads.RECIPIENT_IDS,
        Threads.SNIPPET, Threads.SNIPPET_CHARSET, Threads.READ, Threads.ERROR,
        Threads.HAS_ATTACHMENT, "yl_name", "yl_date", "yl_snippet", "yl_read",
        "yl_error", "yl_count", "yl_has_attachment", "yl_data_type", 
        "case when (case when date is null then 0 else date end)" + 
        " > (case when yl_date is null then 0 else yl_date end)" + 
        " then date else yl_date end AS max_date "
    };
    ...
    private static final int YL_NAME = 9;
    private static final int YL_DATE = 10;
    private static final int YL_SNIPPET = 11;
    private static final int YL_READ = 12;
    private static final int YL_ERROR = 13;
    private static final int YL_COUNT = 14;
    private static final int YL_HAS_ATTACHMENT = 15;
    private static final int YL_DATA_TYPE = 16;
    ...
    private boolean mImslUser;
    private String mImsDataType;
    ...
    public void markImsAsRead() {
        Cursor cursor = null;
        int count = 0;
        final Uri threadUri = getUri();
        String thread = threadUri.getLastPathSegment();
        ContentResolver resolver = mContext.getContentResolver();
        
        try {
            cursor = resolver.query(MessageUtils.PduYlColumns.IMS_URI,
                    null,
                    "read = 0 AND thread_id =" + thread,
                    null,
                    null);
            count = cursor.getCount();            
        } catch (Exception e) {
            Log.e(TAG, "Conversation markImsAsRead error");
        }finally {
            if (cursor != null) {
                cursor.close();                
            }
        ...
        try {
            count = resolver.update(MessageUtils.PduYlColumns.IMS_URI, 
                    values, 
                    "read = 0 AND thread_id =" + thread, 
                    null);            
        } catch (Exception e) {
            Log.e(TAG, "Conversation markImsAsRead update error");
        }
    }
    ...
    public synchronized boolean hasImsUser() {
        return mImslUser;
    }

    public synchronized String getImsDataType() {
        return mImsDataType;
    }
    
    public synchronized boolean getImsState(){
    	return mImsState;
    }
    ...
    public static void startDelete(AsyncQueryHandler handler, int token, boolean deleteAll,
        ...
        Uri uri = ContentUris.withAppendedId(MessageUtils.PduYlColumns.IMS_CONVERSATIONS_URI, threadId);
    ...
    public static void startDelete(AsyncQueryHandler handler, int token, boolean deleteAll,
            String threadId) {
        String strWhere = "thread_id IN(" + threadId + ")";
        if (deleteAll == false) {
         strWhere = strWhere + " AND locked=0";    
        }
      //handler.startDelete(token, threadId, Threads.CONTENT_URI, strWhere, null);
        handler.startDelete(token, threadId, MessageUtils.PduYlColumns.IMS_CONVERSATIONS_URI, strWhere, null);
    }
    ...
    public static void startDeleteAll(AsyncQueryHandler handler, int token, boolean deleteAll) {
        ...
        handler.startDelete(token, null, MessageUtils.PduYlColumns.IMS_CONVERSATIONS_URI, selection, null);
    }
    ...
    public static void startQueryHaveLockedMessages(AsyncQueryHandler handler, long[] threadId,
        ...
        handler.cancelOperation(token);
        Uri uri = MmsSms.CONTENT_LOCKED_URI;
        String strWhere = "thread_id IN(" + CommonMethod.arrayToString(threadId, ",", false) + ")";
        String s = CommonMethod.arrayToString(threadId, ",", false);
        handler.startQuery(token, new String(s), uri,
                ALL_THREADS_PROJECTION, strWhere, null, Conversations.DEFAULT_SORT_ORDER);
    ...
    private static void fillFromCursor(Context context, Conversation conv,
        ...
        synchronized (conv) {
            ...
            conv.mMessageCount += c.getInt(YL_COUNT);
            
            if (c.getInt(YL_COUNT) > 0) {
                
                Long msgLong = c.getLong(DATE);
                Long imsLong = c.getLong(YL_DATE);
                Date msgDate = new Date(msgLong);
                Date imsDate = new Date(imsLong);                
                int tempSnippent = 0;
                
                if (c.getInt(MESSAGE_COUNT) == 0 && c.getInt(YL_COUNT) == 1) {
                	msgLong = new Long(0);
				}
                
                if (msgLong == 0 || imsDate.after(msgDate)) {
                    conv.mDate = imsLong;
                    tempSnippent = YL_SNIPPET;
                    String attachmentType = c.getString(YL_DATA_TYPE);
                    if (!TextUtils.isEmpty(attachmentType)) {
						conv.mImsDataType = attachmentType;
						conv.mHasAttachment = true;
					}else {
						conv.mHasAttachment =false;
						conv.mImsDataType = null;
					}
                }else {
                    conv.mDate = msgLong;
                    tempSnippent = SNIPPET;
					conv.mImsDataType = null;
	                conv.mHasAttachment = (c.getInt(HAS_ATTACHMENT) != 0);               
                }
                // Replace the snippet with a default value if it's empty.
                String snippet = MessageUtils.extractEncStrFromCursor(c, tempSnippent, SNIPPET_CS);
                if (TextUtils.isEmpty(snippet)) {
                    snippet = context.getString(R.string.no_subject_view);
                }
                conv.mSnippet = snippet;
                
                boolean isTrue = c.getInt(READ) == 0 || c.getInt(YL_READ) == 0;
                conv.setHasUnreadMessages(isTrue);
                isTrue = c.getInt(ERROR)!=0 || c.getInt(YL_ERROR) != 0;
                conv.mHasError = (isTrue);
            }else {
    	...
    	int recipientsCount = conv.mRecipients.size();    	
    	conv.mImslUser = false;	
    	conv.mImsState = false;
    	
        if (recipientsCount == 1) {        	
        	String tNumber = MessageUtils.fixPhoneNumber(conv.mRecipients.get(0).getNumber());
        	
        	Cursor cursor = null;
        	try {
        		cursor = context.getContentResolver().query(RosterData.CONTENT_URI, 
															new String[]{RosterData.STATUS}, 
															RosterData.ROSTER_USER_ID + "=" + tNumber, 
															null, 
															null);
        		...
        		if (cursor != null && cursor.getCount() > 0) {
            		...
            		if (cursor.moveToNext()) {
						conv.mImsState = cursor.getInt(0) == 1 ? true : false; 
					}
				}	
        		
        		cursor.close();
			} catch (Exception e) {
				Log.e(TAG, "Conversation fillFromCursor()");
				if (cursor != null) {
					cursor.close();
				}
			}
    ...
    public static void markAllConversationsAsSeen(final Context context) {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                blockingMarkAllImsMessagesAsSeen(context);
                // Always update notifications regardless of the read state.
    ...
    public static void markAllConversationsAsSeenForConversation(final Context context) {
        if (DEBUG) {
            LogTag.debug("Conversation.markAllConversationsAsSeen");
        }
    ...
    private static void blockingMarkAllImsMessagesAsSeen(final Context context) {
        ContentResolver resolver = context.getContentResolver();
        Cursor cursor = resolver.query(MessageUtils.PduYlColumns.IMS_URI,
                SEEN_PROJECTION,
                "seen=0",
                null,
                null);
        ...
        resolver.update(MessageUtils.PduYlColumns.IMS_URI,
                values,
                "seen=0",
                null);
    }

...
import com.lewa.PIM.mms.ui.ClassZeroActivity;
...
//        }
    ...
    private void handleWapPushReceived(Intent intent, int error) {
        ...
        {
            Log.d(TAG, "handleWapPushReceived   mime is application/vnd.wap.sic");
            ...
            if (bundle != null) {
                ...
                service_center = bundle.getString("service_center");
                Log.i(TAG, "handleWapPushReceived: sender=" + sender + ", service_center=" + service_center);
            }

			WapPushParser w = null;
			try
			{
				w = new WapPushParser(pushData);
			}
			catch (Exception ex)
			{
			...
			Log.d(TAG, "SI: " + w.getSI());
			Log.d(TAG, "title: " + w.getTitle());
			Log.d(TAG, "content: " + w.getContent());
			
			Uri pushMessageUri = storeWapPushMessage(this, ((sender != null) ? sender : "WAP PUSH"), w.getTitle() + " " + w.getContent());
			
            if (pushMessageUri != null) {
    ...
    private void handleSmsReceived(Intent intent, int error) {
		...
		// remove by shenqi 
		// move updateNewMessageIndicator to PIM service by listen sms table changed
		// because sometime we can not receive the  SMS_RECEIVED_ACTION witch block by the 3rd app
        //if (messageUri != null) {
            // Called off of the UI thread so ok to block.
           // MessagingNotification.blockingUpdateNewMessageIndicator(this, true, false);
        //}
    ...
    private Uri storeWapPushMessage(Context context, String address, String content)
    {
        ...
        pushMessageCV.put("read", "0"); //0:unread, 1:read
        pushMessageCV.put("status", "-1");
        pushMessageCV.put("body", content);
    
        ContentResolver resolver = context.getContentResolver();
        Uri insertedUri = SqliteWrapper.insert(context, resolver, Inbox.CONTENT_URI, pushMessageCV);
        return insertedUri;
    }
    ...
    private void debugIntent(Intent intent)
    {
        try {
            if (intent == null) {
                Log.d(TAG, "debugIntent  intent = null.");
                return ;
            }
            
            //wap push intent extra [pduType, data, header, transactionId]
            Log.d(TAG, "debugIntent  intent type: " + intent.getType() + ", extra keys: " + intent.getExtras().keySet().toString());
            ...
            while (iterator.hasNext())
            {
                ...
                Log.d(TAG, key + "(" + o.getClass().getSimpleName() +"): " + o.toString());  
            }
            
            byte[] data = intent.getByteArrayExtra("header");
            String buf = byteArray2String(data);
            Log.d(TAG, "debugIntent  header(" + buf.length() + ") " + buf);                       

            data = intent.getByteArrayExtra("data");
            buf = byteArray2String(data);
            Log.d(TAG, "debugIntent  data(" + buf.length() + ") " + buf);                     
        } catch (Exception e) {

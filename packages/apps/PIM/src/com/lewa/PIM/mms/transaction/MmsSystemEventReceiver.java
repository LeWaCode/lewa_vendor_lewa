...
public class MmsSystemEventReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(Mms.Intents.CONTENT_CHANGED_ACTION)) {
            ...
            if("TD".equals(SystemProperties.get("ro.product.network")))
                 handleUpdateSmsStorageLocation();
    ...
    // Add by Fanzhong
    private static final int MMS_LOCATION = 2; // 1. sim 2. database
	private void handleUpdateSmsStorageLocation() {
        SmsManager.getDefault().setNewSmsIndication(MMS_LOCATION);			
		Log.e(TAG,
				"In setSmsStorageIndicator() , mt = " + MMS_LOCATION);
    }
}

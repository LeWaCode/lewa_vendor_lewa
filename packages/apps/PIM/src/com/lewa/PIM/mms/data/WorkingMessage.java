...
import com.lewa.PIM.mms.UserHappinessSignals;
import com.lewa.PIM.mms.ExceedMessageSizeException;
import com.lewa.PIM.mms.LogTag;
import com.lewa.PIM.mms.MmsConfig;
import com.lewa.PIM.mms.ResolutionException;
import com.lewa.PIM.mms.UnsupportContentTypeException;
...
import com.lewa.PIM.mms.transaction.MessageSender;
...
import com.lewa.PIM.mms.ui.SlideshowEditor;
...
public class WorkingMessage {
    ...
    private String mYLFilePath = null;

    public void setIsYLMessage(boolean b){
    	mIsYL = b;
    }
    
    public void setYLFilePath(String path){
    	mYLFilePath = path;
    }
    
    public String getYLFilePath(){
    	return mYLFilePath;
    }
    ...
    private WorkingMessage(NewMessageComposeActivity activity) {
        mActivity = activity;
        mContentResolver = mActivity.getContentResolver();
        mStatusListener = activity;
        mAttachmentType = TEXT;
     ...
     * Creates a new working message.
     */
    public static WorkingMessage createEmpty(NewMessageComposeActivity activity) {
        // Make a new empty working message.
        WorkingMessage msg = new WorkingMessage(activity);
        return msg;
    }
    ...
    public static WorkingMessage load(NewMessageComposeActivity activity, Uri uri) {
        // If the message is not already in the draft box, move it there.
        if (!uri.toString().startsWith(Mms.Draft.CONTENT_URI.toString())) {
            PduPersister persister = PduPersister.getPduPersister(activity);
            if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
    ...
    public static WorkingMessage loadDraft(NewMessageComposeActivity activity,
                                            Conversation conv) {
            WorkingMessage msg = new WorkingMessage(activity);
            if (msg.loadFromConversation(conv)) {
                return msg;
    ...
    */
    ...
    public void send(final String recipientsInUI) {
        ...
        if (requiresMms() || addressContainsEmailToMms(conv, msgTxt)) {
            ...
            final String ylMsgText = mText.toString();
            final String ylNewRecipients = newRecipients;
            final String ylPaht = mYLFilePath;
            new Thread(new Runnable() {
                public void run() {
                	sendIMMessageWorker(ylMsgText, ylPaht, ylNewRecipients, 0, IMService.GeXinIM);
                }
            }).start();
    ...
    private void sendIMMessageWorker(String msgText,String filePaths, String semiSepRecipients,long threadId, int IMType) {
        String[] dests = TextUtils.split(semiSepRecipients, ";");
        LogTag.debug("sendSmsWorker sending message: recipients=" + semiSepRecipients +", threadId=" + threadId +"LoginStatus" + IMClient.getLoginStatus(mActivity,IMMessage.GeXinIM));
        IMMessage senderMsg = new IMMessage(IMType);
        senderMsg.setSendAddress(dests[0]);
        if (!TextUtils.isEmpty(filePaths)) {
            senderMsg.setFileUri(new String[]{Uri.fromFile(new File(filePaths)).toString()});			
		}
        senderMsg.setText(msgText);
        senderMsg.setThreadId("" + threadId);
        mStatusListener.onPreMessageSent();
        try {
            IMClient.sendMessage(mActivity,senderMsg);

    ...
    private void ensureRouteToHost(String url, TransactionSettings settings) throws IOException {
        ...
        if (settings.isProxySet()) {
            ...
            if (inetAddr == -1) {
                ...
                while (!connMgr.requestRouteToHost(
                        ConnectivityManager.TYPE_MOBILE_MMS, inetAddr)) {
                    if (waitCount++ < 60) {
                        try {
                            Log.e("Transaction", "ensureRouteToHost sleep  waitCount=" + waitCount);
                            Thread.currentThread().sleep(1000);
                        } catch (InterruptedException e) {
                            Log.e("Transaction", "e=" + e);
                        }
                    } else {
        ...
        Log.e("Transaction", "ensureRouteToHost  waitCount=" + waitCount);
    }

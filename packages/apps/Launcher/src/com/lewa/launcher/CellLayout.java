...
public class CellLayout extends WidgetCellLayout {
	...
	private final boolean mAutoStretch = false;
	...
	private Drawable mCellBg;
	private boolean mDrawCellBg;
	...
	public CellLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mCellBg = context.getResources().getDrawable(R.drawable.pressed_application_background);
	}
	...
	int getIconWidth(){
	    return mCellWidth + mWidthGap;
	}
	
	int getIconHeight(){
	    return mCellHeight - mHeightGap;
	}
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		try {
			...
			// Someone tried to add a view here without removing it first from
			// its previous parent...
		}
	}
	...
	private static void findIntersectingVacantCells(CellInfo cellInfo, int x,
		...
		// ADW: i can't access current desktop rows/columns, so i try to catch
		// the
	 ...
	 * 
	 * addVacantCell(current, cellInfo); if (current.left > 0) { if
	 * (isColumnEmpty(current.left - 1, current.top, current.bottom, occupied))
	 * { current.left--; findVacantCell(current, xCount, yCount, occupied,
	 * cellInfo); current.left++; } }
	 * 
	 * if (current.right < xCount - 1) { if (isColumnEmpty(current.right + 1,
	 * current.top, current.bottom, occupied)) { current.right++;
	 * findVacantCell(current, xCount, yCount, occupied, cellInfo);
	 * current.right--; } }
	 * 
	 * if (current.top > 0) { if (isRowEmpty(current.top - 1, current.left,
	 * current.right, occupied)) { current.top--; findVacantCell(current,
	 * xCount, yCount, occupied, cellInfo); current.top++; } }
	 * 
	 * if (current.bottom < yCount - 1) { if (isRowEmpty(current.bottom + 1,
	 * current.left, current.right, occupied)) { current.bottom++;
	 * findVacantCell(current, xCount, yCount, occupied, cellInfo);
	 * current.bottom--; } } }
	 */
	// TODO: ADW.
	/**
	 * I don't understand at all why the "findVacantCell" recursive method do
	 * what it does, but seems there's something wrong with it actually works
	 * cause default launchers use a 4x4 grid, but as soon as we rise it to 6x6
	 * or 7x7 (please, think on tablets!!!!) it starts to ANR and act weird
	 * 
	 * Tried a lot of things, and @unekual sent me the following non-recursive
	 * piece of code This does not find ALL the possible vacant cell
	 * combinations, but it works right fine and does not stuck on a stupid ANR.
	 */
	...
	public static boolean isEmpty(int x0, int x1, int y0, int y1,
			boolean[][] occupied) {
		for (int x = x0; x <= x1; x++)
			for (int y = y0; y <= y1; y++)
				if (occupied[x][y])
	...
	CellInfo findAllVacantCellsFromOccupied(boolean[][] occupied,
		...
		// Why s it calling a recursive/loop method from within another
		// loop!!!!????
		// Every time you longpress an item it does the findVacantCell zillions
		// of times
		// eating a lot of cpu/ram!!!!!
		// Just calling this, seems to work, but i'd need further testing....
		// volunteers?
		findVacantCell(current, xCount, yCount, occupied, cellInfo);
...
//				+ getTop();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mPortrait) {
			...
			tmpCellW = (widthSpecSize - mShortAxisStartPadding - mShortAxisEndPadding)
					/ mColumns;
			tmpCellH = (heightSpecSize - mLongAxisStartPadding - mLongAxisEndPadding)
					/ mRows;
		} else {
			...
			tmpCellW = (widthSpecSize - mLongAxisStartPadding - mLongAxisEndPadding)
					/ mColumns;
			tmpCellH = (heightSpecSize - mShortAxisStartPadding - mShortAxisEndPadding)
					/ mRows;
		}
		if (mAutoStretch) {
			mCellWidth = tmpCellW;
	...
	public void findOccupiedCells(int xCount, int yCount,
	...
	static final class CellInfo implements ContextMenu.ContextMenuInfo {
		...
		boolean findCellForSpan(int[] cellXY, int spanX, int spanY,
			...
			for (int i = 0; i < count; i++) {
				...
				if (cell.spanX == spanX && cell.spanY == spanY && (cellXY[0] == cell.cellX && cellXY[1] == cell.cellY)) {
					cellXY[0] = cell.cellX;
					cellXY[1] = cell.cellY;
					found = true;
					break;
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (isAnimating) {
			...
			alpha = easeOut(mCurrentTime, 0, 255, mAnimationDuration);
	...
	public void setDrawCellBg(boolean hasBg) {
	    mDrawCellBg = hasBg;
	}
	
	public void setCellBg(Drawable cellBg){
	    mCellBg = cellBg;
	}
	
	public void setCellBgBounds(Rect rect){
	    mCellBg.setBounds(rect);
	}

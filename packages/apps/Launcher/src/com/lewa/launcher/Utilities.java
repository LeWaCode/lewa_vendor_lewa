...
final class Utilities {
	...
	private static boolean sLewaIcon = false;
	...
	static Drawable createIconThumbnail(Drawable icon, Context context) {
		// Begin [pan 110816 for hvga] modify
		final Resources resources = context.getResources();
		if (sLewaIcon) {
			sIconWidth = sIconHeight = (int) resources.getDimension(R.dimen.app_lewaicon_size);
			sLewaIcon = false;
	...
	static Bitmap createBitmapThumbnail(Bitmap bitmap, Context context) {
		// Begin [pan 110816 for hvga] modify
		/*
		 * if (sIconWidth == -1) { final Resources resources =
		 * context.getResources(); sIconWidth = sIconHeight = (int)
		 * resources.getDimension( android.R.dimen.app_icon_size); }
		 */
		final Resources resources = context.getResources();
		if (sLewaIcon) {
			sIconWidth = sIconHeight = (int) resources
					.getDimension(R.dimen.app_lewaicon_size);
			sLewaIcon = false;
		} else {
			sIconWidth = sIconHeight = (int) resources
					.getDimension(R.dimen.app_icon_size);
	...
	static Drawable drawReflection(Drawable icon, Context context) {
		...
		try {
			reflectionImage = Bitmap.createBitmap(original, 0, height / 2,
		...
		canvas.drawRect(0, height - 6, width, bitmapWithReflection.getHeight(),
				paint);
		...
		try {
			return new FastBitmapDrawable(Bitmap.createScaledBitmap(
					bitmapWithReflection, Math.round(sIconWidth * ratio),
					sIconHeight, true));
		} catch (OutOfMemoryError e) {
			return icon;
		}
	...
	static Drawable scaledDrawable(Drawable icon, Context context,
			boolean tint, float scale, int color) {
		final Resources resources = context.getResources();
		sIconWidth = sIconHeight = (int) resources
				.getDimension(android.R.dimen.app_icon_size);
		...
		if (tint) {
			...
			LinearGradient shader = new LinearGradient(width / 2, 0, width / 2,
					height, Color.argb(220, Color.red(color),
							Color.green(color), Color.blue(color)), Color.argb(
	...
	static void setDefaultValue(boolean islewa) {
		sLewaIcon = islewa;
	}
}

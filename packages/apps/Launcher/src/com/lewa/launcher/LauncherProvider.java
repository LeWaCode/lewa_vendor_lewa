...
public class LauncherProvider extends ContentProvider {
	...
	static final String EXTRA_BIND_SOURCES = "com.lewa.launcher.settings.bindsources";
	static final String EXTRA_BIND_TARGETS = "com.lewa.launcher.settings.bindtargets";
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		private int copyFromCursor(SQLiteDatabase db, Cursor c) {
			...
			final int iconBackgroundIndex = c
					.getColumnIndexOrThrow(LauncherSettings.Favorites.ICON_BACKGROUND);
			...
			while (c.moveToNext()) {
				...
				values.put(LauncherSettings.Favorites.ICON_BACKGROUND,
						c.getString(iconBackgroundIndex));
				rows[i++] = values;
			}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (version < 5) {
				...
				try {
					/*
					 * ComponentName c=new ComponentName(mContext,
					 ...
					 * LauncherSettings.Favorites.ICON_BACKGROUND+ ")"+
					 * "VALUES(" + "'Apps'," +
					 * "'#Intent;action="+CustomShortcutActivity
					 * .ACTION_LAUNCHERACTION+";component="+c.flattenToString()+
					 * ";i.DefaultLauncherAction.EXTRA_BINDINGVALUE=4;end'," +
					 * LauncherSettings.Favorites.CONTAINER_MAB+"," + "-1," +
					 * "-1," + "-1," + "1," + "1," + "1," + "-1," + "0," +
					 * "'"+c.getPackageName()+"'," +
		...
		private boolean addFolder(SQLiteDatabase db, ContentValues values, TypedArray a){
            ...
            int id = a.getInt(R.styleable.Favorite_id, 0);
            String title = a.getString(R.styleable.Favorite_title);
                  
        	values.put(Favorites.ITEM_TYPE, LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER);//Favorites.ITEM_TYPE_APPWIDGET
		...
		private boolean addAppShortcut(SQLiteDatabase db, ContentValues values,
			...
			String title = a.getString(R.styleable.Favorite_id);
			try {
				ComponentName cn;
				try {
					// Begin [pan] modify
					if (title != null) {
						cn = IconBackgroundSetting.getComponentName(title);
				...
				if (isDefault) {
					values.put(Favorites.ICON_BACKGROUND, "iconbg0");
				}
				db.insert(TABLE_FAVORITES, null, values);
        ...
        // Begin, added by zhumeiquan, 20121017
          private boolean addUriShortcut(SQLiteDatabase db, ContentValues values,
            ...
            try {
                ...
                Log.w("launcher", "Shortcut has malformed uri: " + uri);
                return false; // Oh well
            }

            if (iconResId == 0 || titleResId == 0) {
                Log.w("launcher", "Shortcut is missing title or icon resource ID");
                return false;
            }
        ...
        // Begin, zhumeiquan tempraly modify , 20120725
        private boolean addWidget(SQLiteDatabase db, ContentValues values) {
            ComponentName cn = new ComponentName("com.lewa.search",
                    "com.lewa.search.widget.SearchWidgetProvider");
		 ...
		 private boolean addAppWidget(SQLiteDatabase db, ContentValues values, ComponentName cn,
	            ...
	            try {
                    ...
                    values.put(BaseLauncherColumns.TITLE, "LewaSearch");

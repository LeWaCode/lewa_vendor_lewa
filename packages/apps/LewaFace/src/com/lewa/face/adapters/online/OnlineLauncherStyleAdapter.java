...
import com.lewa.face.preview.slide.online.LauncherStyle;
 ...
 *
 */
public class OnlineLauncherStyleAdapter extends ThumbnailOnlineAdapter{
    
    private static final String TAG = OnlineLauncherStyleAdapter.class.getSimpleName();
    
    public OnlineLauncherStyleAdapter(Context context,ArrayList<ThemeBase> themeBases,int themeType){
    ...
    protected void initPath(ThemeBase themeBase) {
        ...
        try {
//            url = new StringBuilder().append(ThemeUtil.THEME_URL).append("/").append(URLEncoder.encode(themeBase.getName(), "UTF-8")).append("/").append("thumbnail_launcher.jpg").toString();
        	url = themeBase.thumbnailpath;
            thumbnailPath = new StringBuilder().append(ThemeConstants.THEME_ONLINE_THUMBNAIL).append("/").append(ThemeConstants.THEME_THUMBNAIL_LAUNCHER_PREFIX).append(ThemeUtil.getNameNoBuffix(themeBase.getPkg())).toString();
        } catch (Exception e) {

...
import com.lewa.face.preview.slide.online.LockScreenStyle;
...
public class OnlineLockScreenStyleAdapter extends ThumbnailOnlineAdapter{
    ...
    private static final String TAG = OnlineLockScreenStyleAdapter.class.getSimpleName();
    
    public OnlineLockScreenStyleAdapter(Context context,ArrayList<ThemeBase> themeBases,int themeType){
    ...
    protected void initPath(ThemeBase themeBase) {
        ...
        try {
//            url = new StringBuilder().append(ThemeUtil.THEME_URL).append("/").append(URLEncoder.encode(themeBase.getName(), "UTF-8")).append("/").append("thumbnail.jpg").toString();
        	url = themeBase.thumbnailpath;
            thumbnailPath = new StringBuilder().append(ThemeConstants.THEME_ONLINE_THUMBNAIL).append("/").append(ThemeConstants.THEME_THUMBNAIL_THUMBNAIL).append(ThemeUtil.getNameNoBuffix(themeBase.getPkg())).toString();
        } catch (Exception e) {

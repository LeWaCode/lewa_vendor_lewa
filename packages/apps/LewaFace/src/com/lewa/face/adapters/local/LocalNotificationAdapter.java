 ...
 *
 */
public class LocalNotificationAdapter extends ThumbnailLocalAdapter {

    public LocalNotificationAdapter(Context context, ArrayList<ThemeBase> themeBases, int themeType, Handler handler) {
    ...
    public void onClick(View v) {
        ...
        intent.setClass(mContext, Notify.class);
    ...
    protected ArrayList<ThemeBase> updateThemeBases() {
        ...
        return ThemeHelper.getThemeBases(mContext, ThemeConstants.THEME_LOCAL, ThemeConstants.NOTIFY);
    }

    @Override
    protected void attachLabel(ThemeBase themeBase, int position) {
        if (sharedPreferences.getString("notify", ThemeConstants.DEFAULT_THEME_PKG).equals(themeBase.getPkg())) {

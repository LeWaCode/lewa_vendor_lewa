...
import com.lewa.face.adapters.local.LocalSettingAdapter;
 ...
 *
 */
public class Setting extends LocalBaseActivity{
    ...
    protected ThumbnailLocalAdapter localAdapterInstance() {
        // TODO Auto-generated method stub
        return new LocalSettingAdapter(mContext,themeBases,ThemeConstants.THEME_LOCAL,handler);
//    	return null;
    }

    @Override
    protected void parseLocalModelInfo(){
        File bootsRoot = new File(ThemeConstants.THEME_MODEL_SETTING_STYLE);
        File[] localBoots = bootsRoot.listFiles(new ThemeFileFilter(ThemeConstants.THEME_MODEL_SETTINGS,ThemeFileFilter.STARTSWITH));
        if((localBoots != null) && (localBoots.length != 0)){
            
            Arrays.sort(localBoots, new ThemeFileSort());
            
            
            ArrayList<ThemeBase> themePkgInfos = ThemeHelper.getThemeBases(mContext, ThemeConstants.THEME_LOCAL, ThemeConstants.SETTING);
            ...
            ThemeHelper.saveThemeBases(mContext, themeBases, ThemeConstants.THEME_LOCAL, ThemeConstants.SETTING);
        }
    }

...
import com.lewa.face.adapters.local.LocalPhoneAdapter;
 ...
 *
 */
public class Phone extends LocalBaseActivity{
    ...
    protected ThumbnailLocalAdapter localAdapterInstance() {
        // TODO Auto-generated method stub
        return new LocalPhoneAdapter(mContext,themeBases,ThemeConstants.THEME_LOCAL,handler);
//    	return null;
    }

    @Override
    protected void parseLocalModelInfo(){
        File bootsRoot = new File(ThemeConstants.THEME_MODEL_PHONE_STYLE);
        File[] localBoots = bootsRoot.listFiles(new ThemeFileFilter(ThemeConstants.THEME_MODEL_PHONE,ThemeFileFilter.STARTSWITH));
        if((localBoots != null) && (localBoots.length != 0)){
            
            Arrays.sort(localBoots, new ThemeFileSort());
            
            
            ArrayList<ThemeBase> themePkgInfos = ThemeHelper.getThemeBases(mContext, ThemeConstants.THEME_LOCAL, ThemeConstants.PHONE);
...
if(themePkg.equals(ThemeConstants.DEFAULT_THEME_PKG)){		
            ...
            ThemeHelper.saveThemeBases(mContext, themeBases, ThemeConstants.THEME_LOCAL, ThemeConstants.PHONE);
        }
    }

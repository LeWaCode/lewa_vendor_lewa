...
import com.lewa.face.adapters.local.LocalNotificationAdapter;
 ...
 *
 */
public class Notify extends LocalBaseActivity{
    ...
    protected ThumbnailLocalAdapter localAdapterInstance() {
        // TODO Auto-generated method stub
        return new LocalNotificationAdapter(mContext,themeBases,ThemeConstants.THEME_LOCAL,handler);
    }

    @Override
    protected void parseLocalModelInfo(){
        File bootsRoot = new File(ThemeConstants.THEME_MODEL_NOTIFY_STYLE);
        File[] localBoots = bootsRoot.listFiles(new ThemeFileFilter(ThemeConstants.THEME_MODEL_NOTIFY,ThemeFileFilter.STARTSWITH));
        if((localBoots != null) && (localBoots.length != 0)){
            
            Arrays.sort(localBoots, new ThemeFileSort());
            
            
            ArrayList<ThemeBase> themePkgInfos = ThemeHelper.getThemeBases(mContext, ThemeConstants.THEME_LOCAL, ThemeConstants.NOTIFY);
...
if(themePkg.equals(ThemeConstants.DEFAULT_THEME_PKG)){		
            ...
            ThemeHelper.saveThemeBases(mContext, themeBases, ThemeConstants.THEME_LOCAL, ThemeConstants.NOTIFY);
        }
    }

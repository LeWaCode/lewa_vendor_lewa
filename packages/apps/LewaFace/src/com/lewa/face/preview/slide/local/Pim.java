...
public class Pim extends SlideBaseActivity {

	private static final String TAG = Pim.class.getSimpleName();
	...
	protected ArrayList<String> getList() {
		...
		File[] files = preview.listFiles(new ThemeFileFilter(
				ThemeConstants.PREVIEW_PIM, ThemeFileFilter.INDEXOF));
	...
	private class ApplyPimTask extends AsyncTask<String, Void, Boolean> {
		...
		protected void onPreExecute() {
			...
			progressDialog.setTitle(R.string.setting_pim_title);
			progressDialog.setMessage(mContext
					.getString(R.string.setting_pim_msg));
			progressDialog.show();
		}
		...
		protected Boolean doInBackground(String... params) {
			...
			boolean result = applyModule(ThemeConstants.THEME_MODEL_PIM_STYLE,
					ThemeConstants.THEME_MODEL_PIM,
					ThemeConstants.THEME_FACE_PIM_TEMP,
					ThemeConstants.THEME_FACE_PIM);
			if(!ThemeUtil.modulesOnApplied.contains(ThemeConstants.THEME_MODEL_PIM)){
				ThemeUtil.modulesOnApplied.add(ThemeConstants.THEME_MODEL_PIM);
			}
			ThemeUtil
					.applyTheme(
							themeBase.getCnName(),
							ThemeUtil.modulesOnApplied);
		...
		protected void onPostExecute(Boolean result) {
			...
			if (result) {
				...
				editor.putString("pim", themeBase.getPkg()).commit();
